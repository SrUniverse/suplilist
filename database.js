...
// [LIMPEZA] Removida função sanitize duplicada (use escapeHTML de utils.js)
/**
 * Utilitário para congelamento profundo de objetos e arrays.
 * Garante imutabilidade em tempo de execução para metadados críticos.
 */
...
// ─────────────── TERMS DATES ───────────────
export function getTermsUpdatedDate() {
  return "Maio de 2024"; // Data fixa da última revisão real do documento
}

export function getTermsRevisionDate() {
  const date = new Date();
...
/** Mapeamento de IDs de suplementos por objetivo terapêutico (Imutável) */
export const GOAL_MAP = deepFreeze({
  ...
  metabolismo:[37,38,39,54,55,12], // Beta-alanina (12) incluída por relevância metabólica
  longevidade:[25,28,45,52,56,37,57],
});

/** Mapeamento programático de Score para Nível de Evidência (Info #22) */
export const EV_TO_LEVEL = { 5: 'A', 4: 'B', 3: 'C', 2: 'C', 1: 'C' };

/** Definições de prioridade para ordenação (Imutável) */
export const PRIO = Object.freeze({alta:0,media:1,baixa:2,extra:3});

/** Labels amigáveis de prioridade (Imutável) */
export const PLBL = Object.freeze({alta:'Alta Prioridade',media:'Média Prioridade',baixa:'Baixa Prioridade',extra:'Extras Opcionais'});

/** Classes CSS por nível de prioridade (Imutável) */
export const PCLS = Object.freeze({alta:'da',media:'dm',baixa:'db',extra:'de'});

export const AFF = Object.freeze({
  amazonTag:'suplilist-20',
  mlLabel:'suplilist',
  mlCreatine:'https://meli.la/2xQTHcw',
});
export function amazonAff(url) {
  try{
    const u=new URL(url);
...
export function mlAff(url, itemId = 'suplilist') {
  if (!url) return '';
  try{
    const u=new URL(url);
...
export function shopeeAff(url, itemId = 'suplilist') {
  if (!url) return '';
  try {
    const u = new URL(url);
...
export function utm(url, src, med, camp, content) {
  if (!url) return '';
  try {
    const u = new URL(url);
...
export function searchUrl(name, mkt) {
...
const MARKET_MARGINS = { ML: 1.08, AMAZON: 1.18 };

/** Cálculo seguro com fallbacks para evitar erros de renderização (NaN) */
/** [QA] Blindagem contra NaN em cálculos financeiros */
export function mlPrice(i) { 
  const base = Number(i?.pm) || 20;
  return i?.mlp ? Number(i.mlp) : Math.round(base * MARKET_MARGINS.ML); 
}
export function azPrice(i) { 
  const base = Number(i?.pm) || 20;
  return i?.azp ? Number(i.azp) : Math.round(base * MARKET_MARGINS.AMAZON); 
}
export function bestMarketplacePrice(i) { 
  if (!i) return 0;
  const p1 = Number(i.pm) || Infinity;
  const p2 = mlPrice(i) || Infinity;
  const p3 = azPrice(i) || Infinity;
  const best = Math.min(p1, p2, p3);
...
/** Base de dados principal de suplementos.
 * Nota: Não congelado na definição para permitir a injeção de links de afiliados no startup. */
export const IT = [
...
const imgMap = {
  1: 'maca_peruana', 2: 'feno_grego', 3: 'l_citrulina', 4: 'ashwagandha', 5: 'tongkat_ali',
  6: 'boro', 7: 'mucuna_pruriens', 8: 'zinco_bisglicinato', 9: 'magnesio_glicinato', 10: 'vitamina_d3_k2',
  11: 'creatina', 12: 'beta_alanina', 13: 'cafeina_teanina', 14: 'hmb', 15: 'whey_protein',
  16: 'eaa', 17: 'ecdisterona', 18: 'lions_mane', 19: 'bacopa_monnieri', 20: 'rhodiola_rosea',
  21: 'alpha_gpc', 22: 'l_teanina', 23: 'omega_3', 24: 'magnesio_treonato', 25: 'coenzima_q10',
  26: 'beta_glucana', 27: 'vitamina_c', 28: 'quercetina', 29: 'catuaba', 30: 'marapuama',
  31: 'saw_palmetto', 32: 'panax_ginseng', 33: 'taurina', 34: 'shatavari', 35: 'probiotico',
  36: 'psyllium', 37: 'berberina', 38: 'cromo_picolinato', 39: 'cha_verde', 40: 'colageno',
  41: 'ferro_bisglicinato', 42: 'cranberry', 43: 'melatonina', 44: 'glucosamina_condroitina', 45: 'nac',
  46: 'glicina', 47: 'inositol', 48: 'spirulina', 49: 'calcio_citrato_d3', 50: 'valeriana',
  51: 'apigenina', 52: 'resveratrol', 53: 'oleo_de_primula', 54: 'l_carnitina', 55: 'tirosina',
  56: 'curcumina', 57: 'nmn'
};
...
IT.forEach(item => {
  if (item && item.id) {
    const fileName = imgMap[item.id] || `suplemento_${item.id}`;
    _SUPP_IMGS[item.id] = `${BASE_URL}${fileName}.png`;
  }
});
/** Mapeamento de caminhos de imagens dos suplementos (Imutável) */
export const SUPP_IMGS = Object.freeze(_SUPP_IMGS);

/** Guia de interações, riscos e sinergias (Imutável) */
export const RECIPE_SYNERGIES = deepFreeze([
  // [ids[], 'Título', 'Nota explicativa']
  // IDs baseados no banco de dados IT

  // ── Libido & Testosterona ──
  [[4, 5],   'Ashwagandha + Tongkat Ali',        'Combinação sinérgica para testosterona: Ashwagandha reduz cortisol enquanto Tongkat Ali estimula produção direta. Efeito cumulativo superior ao uso isolado.'],
  [[1, 2],   'Maca Preta + Feno-grego',           'Maca age no eixo HPA enquanto Feno-grego inibe aromatase. Juntos cobrem libido hormonal e não-hormonal.'],
  [[3, 1],   'L-Citrulina + Maca',               'Citrulina melhora fluxo sanguíneo via óxido nítrico; Maca potencializa o drive libidinal. Combo clássico para performance sexual.'],
  [[5, 6],   'Tongkat Ali + Boron',              'Tongkat Ali libera testosterona presa ao SHBG enquanto Boro reduz SHBG diretamente. Sinergia documentada para testosterona livre.'],
  [[4, 8, 9],'Ashwagandha + Zinco + Magnésio',   'O trio base para testosterona: Ashwagandha reduz cortisol, Zinco é cofator da síntese e Magnésio reduz SHBG. Stack hormonal completo.'],
  [[29, 30], 'Catuaba + Marapuama',              'Combinação tradicional amazônica para potência sexual. Ação sinérgica no sistema nervoso autônomo e circuito dopaminérgico.'],

  // ── Performance & Hipertrofia ──
  [[11, 15], 'Creatina + Whey Protein',          'O par mais estudado do esporte. Creatina maximiza força e volume de treino; Whey entrega aminoácidos para síntese proteica. Efeitos independentes e aditivos.'],
  [[11, 12], 'Creatina + Beta-Alanina',          'Sinergia comprovada em meta-análises: Creatina melhora potência anaeróbica; Beta-Alanina aumenta tampão de carnosina. Treinos mais intensos e mais longos.'],
  [[13, 22], 'Cafeína + L-Teanina',             'O combo mais estudado para performance cognitiva e física. Teanina elimina a ansiedade da cafeína e prolonga clareza mental. Proporção ideal 1:1.'],
  [[15, 16], 'Whey + EAA',                       'EAA durante o treino + Whey pós-treino: cobertura completa da síntese proteica. Especialmente útil em treinos em jejum.'],
  [[11, 17], 'Creatina + Ecdisterona',           'Ecdisterona ativa síntese proteica via receptor ER-beta; Creatina fornece o substrato energético. Anabolismo natural sinérgico sem supressão hormonal.'],

  // ── Cognição & Energia Mental ──
  [[18, 21], "Lion's Mane + Alpha-GPC",         "Lion's Mane estimula NGF e neurogênese enquanto Alpha-GPC eleva acetilcolina disponível. Combo premium para neuroplasticidade e memória."],
  [[19, 20], 'Bacopa + Rhodiola',               'Bacopa constrói memória de longo prazo (8–12 semanas); Rhodiola oferece clareza mental imediata (dias). Cobertura cognitiva de curto e longo prazo.'],
  [[18, 19], "Lion's Mane + Bacopa",            'Ambos atuam na plasticidade sináptica por vias distintas: NGF e acetilcolina. Stack preferido de neurohackers para aprendizado intenso.'],
  [[22, 13], 'L-Teanina + Cafeína',             'Ver: Cafeína + L-Teanina. Combo bidirecional — pode ser iniciado por qualquer um.'],
  [[21, 24], 'Alpha-GPC + Magnésio Treonato',   'Alpha-GPC eleva acetilcolina; Magnésio Treonato aumenta densidade sináptica no hipocampo. Sinapse mais rica em neurotransmissores com mais receptores disponíveis.'],

  // ── Saúde Geral & Imunidade ──
  [[10, 23], 'Vitamina D3+K2 + Ômega-3',        'D3 e Ômega-3 compartilham a mesma via de absorção (lipídica) e têm efeitos anti-inflamatórios complementares. Combinação cardiovascular e imune de referência.'],
  [[26, 27], 'Beta-Glucana + Vitamina C',        'Beta-Glucana ativa macrófagos e NK cells; Vitamina C potencializa neutrófilos e síntese de colágeno. Imunidade inata e adaptativa reforçadas juntas.'],
  [[28, 8],  'Quercetina + Zinco',              'Quercetina é ionóforo de zinco — facilita entrada de Zn²⁺ nas células. Sinergia antiviral documentada: Quercetina abre o canal, Zinco inibe replicação viral.'],
  [[23, 25], 'Ômega-3 + CoQ10',                'Ambos suportam função mitocondrial por vias distintas. Ômega-3 fluidifica membranas; CoQ10 otimiza a cadeia respiratória. Combo cardio-protetor de primeira linha.'],
  [[9, 10],  'Magnésio + Vitamina D3',          'Magnésio é cofator obrigatório para ativação da Vitamina D. Sem magnésio adequado, suplementar D3 é ineficiente. Sinergia metabólica essencial.'],
]);

export const INTERACT = deepFreeze([
...
/** Diretrizes para ciclos de uso e pausas (Imutável) */
export const CYCLES = deepFreeze([
...
/** Regras lógicas para cálculo de dosagem personalizada (Imutável) */
export const DOSE_RULES = deepFreeze({
...
/** Mapeamento de labels por sexo (Imutável) */
export const SEX_LABEL = Object.freeze({ masculino:'Masculino', feminino:'Feminino' });

// ══════════════ ESTUDOS CIENTÍFICOS ══════════════
/** Compêndio científico detalhado por suplemento (Imutável) */
export const STUDIES = deepFreeze({
  1:{
    ev:4,
  ...
  30:{
    ...
    estudos:[
      ...
      {tipo:'Estudo clínico',ano:'1994',journal:'Am J Nat Med',titulo:'Marapuama e disfunção erétil (estudo Waynberg)',achado:'Melhora em 51% dos casos de DE e aumento de libido em 62% dos participantes',detalhe:'262 homens com DE ou libido reduzida. 1–1,5g/dia por 2 semanas. Estudo aberto, sem controle. Base histórica da indicação.',pmid:null},
...
// ══════════════ [#32] FORMAS COMPARATIVAS ══════════════
export const COMPREHENSIVE_FORMS = deepFreeze({
  magnesio: {
    title: "Variações de Magnésio",
    variants: [
      { name: "Bisglicinato (Quelato)", absorption: "Alta", focus: "Sono, Relaxamento Muscular, Ansiedade", bio: "Melhor estabilidade, alta tolerância gástrica." },
      { name: "Malato", absorption: "Alta", focus: "Energia, Fadiga, Dores Musculares", bio: "Ácido málico auxilia na produção de ATP celular." },
      { name: "Treonato", absorption: "Média", focus: "Foco, Memória, Saúde Cognitiva", bio: "Capaz de atravessar a barreira hematoencefálica." }
    ]
  },
  creatina: {
    title: "Tipos de Creatina",
    variants: [
      { name: "Monohidratada", absorption: "Alta", focus: "Força, Volume, Explosão", bio: "A forma mais estudada e custo-efetiva." },
      { name: "Micronizada", absorption: "Mais Rápida", focus: "Dissolução, Conforto Gástrico", bio: "Partículas menores para evitar desconforto GI." },
      { name: "HCL", absorption: "Instantânea", focus: "Baixa Retenção, Alta Solubilidade", bio: "Exige doses menores para o mesmo efeito." }
    ]
  }
});

// ══════════════ [#33] MATRIZ DE INTERAÇÃO CRUZADA ══════════════
export const CROSS_INTERACTIONS = deepFreeze([
  {
    ids: [41, 49], // Ferro e Cálcio
    type: "competition",
    severity: "warning",
  ...
  {
    ids: [10, 11], // Vitamina D3 e Creatina (Exemplo de Sinergia de Performance)
    type: "synergy",
    severity: "success",
...
/** Base de dados de Perguntas Frequentes (Imutável) */
export const FAQ_DATA = deepFreeze([
