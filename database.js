...
export const getGoalLabel = (goalKey) => {
  const key = `GOAL_${goalKey.toUpperCase()}`;
  return t(key);
};
...
const _SUPP_IMGS = {};
IT.forEach(item => {
  _SUPP_IMGS[item.id] = `${BASE_URL}suplemento_${item.id}.png`;
});
...
/** Regras de sinergia para o gerador de receitas (Imutável) */
export const RECIPE_SYNERGIES = deepFreeze([
  // IDs verificados contra IT em data.js:
  // 13=Cafeína+L-Teanina, 22=L-Teanina isolada
  [[13,22],'Cafeína + L-Teanina','A combinação mais validada para foco limpo sem jitters.'],
  // 4=Ashwagandha KSM-66, 9=Magnésio glicinato
  [[4,9],'Ashwagandha + Magnésio','Potencializa relaxamento e redução de cortisol.'],
  // 5=Tongkat Ali, 6=Boron, 9=Magnésio
  [[5,6,9],'Tongkat Ali + Boron + Magnésio','Trio hormonal: testosterona, SHBG e sono profundo.'],
  // 18=Lion\'s Mane, 19=Bacopa Monnieri, 21=Alpha-GPC
  [[18,19,21],'Lion\'s Mane + Bacopa + Alpha-GPC','Stack cognitivo completo: NGF + memória + acetilcolina.'],
  // 11=Creatina, 15=Whey Protein, 14=HMB
  [[11,15,14],'Creatina + Whey + HMB','Trio anabólico: ATP + síntese proteica + proteção muscular.'],
  // 27=Vitamina C, 28=Quercetina, 8=Zinco
  [[27,28,8],'Vitamina C + Quercetina + Zinco','Imunidade sinérgica: C+Q aumenta absorção de zinco.'],
  // 10=Vitamina D3+K2 (combo já embutido no mesmo produto)
  [[10,9],'Vitamina D3+K2 + Magnésio','D3 mobiliza cálcio; K2 direciona para os ossos; Magnésio ativa D3.'],
  // 23=Ômega-3, 56=Curcumina
  [[23,56],'Ômega-3 + Curcumina','Dupla anti-inflamatória com ação complementar.'],
]);

/** Regras de alertas de segurança para o gerador de receitas (Imutável) */
export const RECIPE_DANGERS = deepFreeze([
  {ids:[7],names:['mucuna'],cond:'imao',msg:'Mucuna + IMAO/antidepressivos → risco de síndrome serotoninérgica.'},
