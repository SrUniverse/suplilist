var T=Object.defineProperty;var z=(l,e,a)=>e in l?T(l,e,{enumerable:!0,configurable:!0,writable:!0,value:a}):l[e]=a;var h=(l,e,a)=>z(l,typeof e!="symbol"?e+"":e,a);import{s as d,A as b}from"./main-7QosEGf5.js";import{S as M}from"./stack-recommender-b07295YU.js";class q{constructor(){h(this,"FIXED_DOSE_SUPPLEMENTS",new Set(["vitamina-d3","omega-3","magnesio-bisglicinato","vitamina-c","ashwagandha"]));h(this,"WEIGHT_BASED_SUPPLEMENTS",new Map([["creatina-monohidratada",.07],["whey-protein",.4],["beta-alanina",.05],["l-carnitina",.02],["cafeina",3]]));h(this,"ACTIVITY_MULTIPLIERS",{low:.85,moderate:1,high:1.15});h(this,"OBJECTIVE_MULTIPLIERS",{bulk:1.2,strength:1.2,cut:.9,endurance:1,general:1});h(this,"TIMING_SCHEDULES",{preWorkout:"30-45 minutos antes do treino",postWorkout:"Pós-treino ou a qualquer hora com carboidratos",morning:"Pela manhã com refeição",night:"30-60 minutos antes de dormir"})}calculate(e,a){if(!e||!a)throw new Error("[DosageCalculator] Supplement and UserProfile are mandatory inputs");const t=a.weight||70,r=a.trainingFrequency||3,i=a.objective||"general",c=a.age||25;let o=e.dosage?.maintenance||0,s=e.dosage?.isFixed||this.FIXED_DOSE_SUPPLEMENTS.has(e.id),p="Dosagem clínica fixa recomendada.";if(!s){const C=this.WEIGHT_BASED_SUPPLEMENTS.get(e.id)||e.dosage.multiplier||.05,k=this._getActivityLevel(r),I=this.ACTIVITY_MULTIPLIERS[k]||1,A=this.OBJECTIVE_MULTIPLIERS[i]||1,R=this._getAgeMultiplier(c);o=t*C*I*A*R,p=`Cálculo biométrico baseado no peso corporal (${t}kg), frequência de treinos (${r}x/semana) e objetivo de ${i}.`}const n=e.dosage?.upperLimit||o*2;let g=!0;o>=n&&(o=n,g=!1);const u=e.dosage?.unit||"g";o=(u==="g"?1:0)===1?Math.round(o*10)/10:Math.round(o);const f=o*7,x=o*30,y=this._getDosageFrequency(e,i),_=e.dosage?.timing||"A qualquer hora",w=e.id==="vitamina-d3"||e.id==="omega-3",S=e.id==="creatina-monohidratada"?"Beba com pelo menos 300ml de água e mantenha alta ingestão hídrica diária (3L+)":"Beba com um copo de água (200ml)",L=e.id==="creatina-monohidratada"?"A creatina funciona por saturação acumulativa, consuma todos os dias inclusive nos dias de descanso.":null;let v=null;e.dosage?.loading&&(v={dose:e.dosage.loading,unit:u,duration:"5-7 dias",frequency:"Fracionado 4x ao dia",description:`Protocolo opcional de saturação inicial: tome ${e.dosage.loading}${u} por dia durante 5-7 dias para saturar os estoques musculares rapidamente, depois retorne à dose de manutenção de ${o}${u}.`});const E=this._buildRationale(e,o,u,t,i,s),$=this._generateWarnings(e,o,n,c);return{daily:o,unit:u,weekly:f,monthly:x,frequency:y,timing:_,withFood:w,withWater:S,note:L,loadingProtocol:v,withinSafetyLimits:g,upperLimit:n,rationale:E,warnings:$,methodology:p}}calculateStack(e,a){return Array.isArray(e)?e.map(t=>({supplementId:t.id,supplementName:t.name,dosage:this.calculate(t,a)})):[]}calculateStackCost(e,a){if(!Array.isArray(e))return 0;let t=0;return e.forEach(r=>{const i=this.calculate(r,a),c=r.pricePerGram||.05;let o=i.daily;i.unit==="mg"?o=i.daily/1e3:i.unit==="UI"&&(o=i.daily*25e-6),t+=o*30*c}),Math.round(t*100)/100}_getActivityLevel(e){return e>=5?"high":e>=3?"moderate":"low"}_getAgeMultiplier(e){return e>=60?.9:1}_getDosageFrequency(e,a){return e.id==="cafeina"?"Somente nos dias de treino (pré-treino)":"Diariamente"}_buildRationale(e,a,t,r,i,c){return c?`Dose fixa padrão de ${a}${t} recomendada clinicamente, sem necessidade de alteração baseada no peso corporal.`:`Dose diária sugerida de ${a}${t} calculada com base no seu peso corporal (${r}kg) e no objetivo de ${i}.`}_generateWarnings(e,a,t,r){const i=[];return r<18&&i.push({type:"warning",message:"Atenção: Suplemento recomendado para maiores de 18 anos. Consulte um médico ou nutricionista."}),a>=t*.9&&i.push({type:"caution",message:"Cuidado: Sua dosagem está muito próxima ao limite máximo de segurança estabelecido."}),i}}const D=new q,P=[{value:"sedentary",label:"Sedentário"},{value:"moderate",label:"Moderado"},{value:"active",label:"Ativo"},{value:"athlete",label:"Atleta"}],j=[{value:"bulk",label:"Bulk"},{value:"cut",label:"Cut"},{value:"strength",label:"Força"},{value:"endurance",label:"Resistência"},{value:"general",label:"Saúde Geral"}],m={A:{bg:"rgba(34,197,94,0.12)",color:"#22C55E"},B:{bg:"rgba(245,158,11,0.12)",color:"#F59E0B"},C:{bg:"rgba(163,163,163,0.12)",color:"#9A9A9A"},D:{bg:"rgba(163,163,163,0.12)",color:"#9A9A9A"}};class V{constructor(e){this.container=e,this._weight=d.user?.weight??75,this._bodyfat=d.user?.bodyfat??null,this._activityLevel="moderate",this._objective=d.user?.objective??"general",this._searchQuery="",this._selectedSupp=null,this._phase="maintenance",this._calcResult=null,this._debounce=null,this._allSupps=M??[]}mount(){this._attachStyles(),this._render(),this._attachListeners()}unmount(){clearTimeout(this._debounce)}_render(){this.container.innerHTML=`
    ...
    `}_renderChips(){const e=this._searchQuery.toLowerCase().trim(),a=e?this._allSupps.filter(t=>t.name.toLowerCase().includes(e)||(t.category??"").toLowerCase().includes(e)):this._allSupps;return a.length?a.map(t=>{const r=this._selectedSupp?.id===t.id,i=t.evidenceLevel??"D",c=m[i]??m.D;return`
        <button class="calcp-chip${r?" calcp-chip--active":""}"
          role="option" aria-selected="${r}"
          data-supp-id="${t.id}" title="${t.name}">
          <span class="calcp-chip-name">${t.name}</span>
          <span class="calcp-chip-cat">${t.category??""}</span>
        ...
        </div>`;const e=this._selectedSupp,a=this._calcResult,t=e.evidenceLevel??"D",r=m[t]??m.D;let i="—",c=e.dosage?.unit??"g";a?i=this._phase==="loading"?a.loading??a.daily??"—":a.daily??a.maintenance??"—":e.dosage&&(i=this._phase==="loading"?e.dosage.loading??e.dosage.maintenance??"—":e.dosage.maintenance??"—");const o=a?.rationale??e.dosage?.timing??"",s=e.safetyScore??0,p=!!(e.dosage?.loading||a?.loading),n=(d.stack??[]).some(g=>g.supplementId===e.id);return`
      <!-- Result header -->
      ...
      <button class="calcp-btn-add${n?" calcp-btn-add--in":""}"
        id="btn-add-protocol" data-supp-id="${e.id}" data-supp-name="${e.name}">
      ...
      <div class="calcp-sci-card">
        ...
        <div class="calcp-sci-section">
          ...
          <p class="calcp-sci-text">${o||"Dosagem baseada em estudos clínicos controlados."}</p>
        </div>
        ...
        ${e.dosage?.timing?`
          <div class="calcp-sci-section">
            ...
            <p class="calcp-timing-text">⏱ ${e.dosage.timing}</p>
          </div>`:""}
      </div>
    `}_calculate(){if(!this._selectedSupp){this._calcResult=null;return}const e={weight:this._weight,bodyfat:this._bodyfat,activityLevel:this._activityLevel,objective:this._objective,trainingFrequency:this._activityLevel==="athlete"?6:this._activityLevel==="active"?5:this._activityLevel==="moderate"?3:2};try{const a=D.calculate(this._selectedSupp,e);this._calcResult=a??null}catch(a){console.warn("[CalculatorPage] dosage calc error",a),this._calcResult=null}}_refreshResult(){clearTimeout(this._debounce),this._debounce=setTimeout(()=>{this._calculate();const e=this.container.querySelector("#result-card");e&&(e.innerHTML=this._renderResult()),this._attachResultListeners()},80)}_refreshChips(){const e=this.container.querySelector("#supp-list");e&&(e.innerHTML=this._renderChips())}_attachListeners(){this.container.querySelector("#inp-weight")?.addEventListener("input",e=>{const a=parseFloat(e.target.value);!isNaN(a)&&a>=40&&a<=200&&(this._weight=a,this._refreshResult())}),this.container.querySelector("#inp-bodyfat")?.addEventListener("input",e=>{const a=parseFloat(e.target.value);this._bodyfat=isNaN(a)?null:a,this._refreshResult()}),this.container.querySelector("#sel-activity")?.addEventListener("change",e=>{this._activityLevel=e.target.value,this._refreshResult()}),this.container.querySelector("#sel-objective")?.addEventListener("change",e=>{this._objective=e.target.value,this._refreshResult()}),this.container.querySelector("#inp-search")?.addEventListener("input",e=>{this._searchQuery=e.target.value,this._refreshChips(),this._attachChipListeners()}),this._attachChipListeners(),this._attachResultListeners()}_attachChipListeners(){this.container.querySelectorAll("[data-supp-id]").forEach(e=>{e.classList.contains("calcp-chip")&&e.addEventListener("click",()=>{const a=e.dataset.suppId;this._selectedSupp?.id===a?this._selectedSupp=null:this._selectedSupp=this._allSupps.find(t=>t.id===a)??null,this._refreshChips(),this._attachChipListeners(),this._refreshResult()})})}_attachResultListeners(){this.container.querySelectorAll("[data-phase]").forEach(a=>{a.addEventListener("click",()=>{this._phase=a.dataset.phase;const t=this.container.querySelector("#result-card");t&&(t.innerHTML=this._renderResult()),this._attachResultListeners()})});const e=this.container.querySelector("#btn-add-protocol");e&&e.addEventListener("click",()=>{const a=e.dataset.suppId,t=e.dataset.suppName;if((d.stack??[]).some(c=>c.supplementId===a))d.dispatch(b.REMOVE_FROM_STACK,{supplementId:a}),window.SupliToast&&window.SupliToast.show(`${t} removido do protocolo`,"info");else{const c=this._selectedSupp,o=this._calcResult,s=this._phase==="loading"?o?.loading??o?.daily??c?.dosage?.loading??c?.dosage?.maintenance??0:o?.daily??c?.dosage?.maintenance??0,p=c?.dosage?.unit??"g";d.dispatch(b.ADD_TO_STACK,{supplementId:a,name:t,dosage:s,unit:p,frequency:"diário"}),window.SupliToast&&window.SupliToast.show(`✓ ${t} adicionado ao protocolo!`,"success")}const i=this.container.querySelector("#result-card");i&&(i.innerHTML=this._renderResult()),this._attachResultListeners()})}_attachStyles(){if(document.getElementById("calcp-styles"))return;const e=document.createElement("style");e.id="calcp-styles",e.textContent=`
      /* Root */
      .calcp-root {
        display: flex;
