var z=Object.defineProperty;var M=(l,e,a)=>e in l?z(l,e,{enumerable:!0,configurable:!0,writable:!0,value:a}):l[e]=a;var h=(l,e,a)=>M(l,typeof e!="symbol"?e+"":e,a);import{s as d,A as b}from"./main-Q5JJQYYZ.js";import{S as q}from"./stack-recommender-b07295YU.js";import{e as f}from"./escape-Br5wU8qn.js";import{E as P}from"./evidence-D5RtUc7g.js";class D{constructor(){h(this,"FIXED_DOSE_SUPPLEMENTS",new Set(["vitamina-d3","omega-3","magnesio-bisglicinato","vitamina-c","ashwagandha"]));h(this,"WEIGHT_BASED_SUPPLEMENTS",new Map([["creatina-monohidratada",.07],["whey-protein",.4],["beta-alanina",.05],["l-carnitina",.02],["cafeina",3]]));h(this,"ACTIVITY_MULTIPLIERS",{low:.85,moderate:1,high:1.15});h(this,"OBJECTIVE_MULTIPLIERS",{bulk:1.2,strength:1.2,cut:.9,endurance:1,general:1});h(this,"TIMING_SCHEDULES",{preWorkout:"30-45 minutos antes do treino",postWorkout:"Pós-treino ou a qualquer hora com carboidratos",morning:"Pela manhã com refeição",night:"30-60 minutos antes de dormir"})}calculate(e,a){if(!e||!a)throw new Error("[DosageCalculator] Supplement and UserProfile are mandatory inputs");const t=a.weight||70,r=a.trainingFrequency||3,i=a.objective||"general",c=a.age||25;let o=e.dosage?.maintenance||0,s=e.dosage?.isFixed||this.FIXED_DOSE_SUPPLEMENTS.has(e.id),p="Dosagem clínica fixa recomendada.";if(!s){const k=this.WEIGHT_BASED_SUPPLEMENTS.get(e.id)||e.dosage?.multiplier||.05,I=this._getActivityLevel(r),R=this.ACTIVITY_MULTIPLIERS[I]||1,T=this.OBJECTIVE_MULTIPLIERS[i]||1,A=this._getAgeMultiplier(c);o=t*k*R*T*A,p=`Cálculo biométrico baseado no peso corporal (${t}kg), frequência de treinos (${r}x/semana) e objetivo de ${i}.`}const n=e.dosage?.upperLimit||o*2;let g=!0;o>=n&&(o=n,g=!1);const u=e.dosage?.unit||"g";o=(u==="g"?1:0)===1?Math.round(o*10)/10:Math.round(o);const x=o*7,y=o*30,_=this._getDosageFrequency(e,i),w=e.dosage?.timing||"A qualquer hora",S=e.id==="vitamina-d3"||e.id==="omega-3",L=e.id==="creatina-monohidratada"?"Beba com pelo menos 300ml de água e mantenha alta ingestão hídrica diária (3L+)":"Beba com um copo de água (200ml)",E=e.id==="creatina-monohidratada"?"A creatina funciona por saturação acumulativa, consuma todos os dias inclusive nos dias de descanso.":null;let v=null;e.dosage?.loading&&(v={dose:e.dosage.loading,unit:u,duration:"5-7 dias",frequency:"Fracionado 4x ao dia",description:`Protocolo opcional de saturação inicial: tome ${e.dosage.loading}${u} por dia durante 5-7 dias para saturar os estoques musculares rapidamente, depois retorne à dose de manutenção de ${o}${u}.`});const $=this._buildRationale(e,o,u,t,i,s),C=this._generateWarnings(e,o,n,c);return{daily:o,unit:u,weekly:x,monthly:y,frequency:_,timing:w,withFood:S,withWater:L,note:E,loadingProtocol:v,withinSafetyLimits:g,upperLimit:n,rationale:$,warnings:C,methodology:p}}calculateStack(e,a){return Array.isArray(e)?e.map(t=>({supplementId:t.id,supplementName:t.name,dosage:this.calculate(t,a)})):[]}calculateStackCost(e,a){if(!Array.isArray(e))return 0;let t=0;return e.forEach(r=>{const i=this.calculate(r,a),c=r.pricePerGram||.05;let o=i.daily;i.unit==="mg"?o=i.daily/1e3:i.unit==="UI"&&(o=i.daily*25e-6),t+=o*30*c}),Math.round(t*100)/100}_getActivityLevel(e){return e>=5?"high":e>=3?"moderate":"low"}_getAgeMultiplier(e){return e>=60?.9:1}_getDosageFrequency(e,a){return e.id==="cafeina"?"Somente nos dias de treino (pré-treino)":"Diariamente"}_buildRationale(e,a,t,r,i,c){return c?`Dose fixa padrão de ${a}${t} recomendada clinicamente, sem necessidade de alteração baseada no peso corporal.`:`Dose diária sugerida de ${a}${t} calculada com base no seu peso corporal (${r}kg) e no objetivo de ${i}.`}_generateWarnings(e,a,t,r){const i=[];return r<18&&i.push({type:"warning",message:"Atenção: Suplemento recomendado para maiores de 18 anos. Consulte um médico ou nutricionista."}),a>=t*.9&&i.push({type:"caution",message:"Cuidado: Sua dosagem está muito próxima ao limite máximo de segurança estabelecido."}),i}}const j=new D,N=[{value:"sedentary",label:"Sedentário"},{value:"moderate",label:"Moderado"},{value:"active",label:"Ativo"},{value:"athlete",label:"Atleta"}],B=[{value:"bulk",label:"Bulk"},{value:"cut",label:"Cut"},{value:"strength",label:"Força"},{value:"endurance",label:"Resistência"},{value:"general",label:"Saúde Geral"}],m=P;class W{constructor(e){this.container=e,this._weight=d.user?.weight??75,this._bodyfat=d.user?.bodyfat??null,this._activityLevel="moderate",this._objective=d.user?.objective??"general",this._searchQuery="",this._selectedSupp=null,this._phase="maintenance",this._calcResult=null,this._debounce=null,this._allSupps=q??[]}mount(){this._attachStyles(),this._render(),this._attachListeners()}unmount(){clearTimeout(this._debounce)}_render(){this.container.innerHTML=`
    ...
    `}_renderChips(){const e=this._searchQuery.toLowerCase().trim(),a=e?this._allSupps.filter(t=>t.name.toLowerCase().includes(e)||(t.category??"").toLowerCase().includes(e)):this._allSupps;return a.length?a.map(t=>{const r=this._selectedSupp?.id===t.id,i=t.evidenceLevel??"D",c=m[i]??m.C;return`
      ...
      <div class="calcp-sci-card">
        ...
        <div class="calcp-sci-section">
          ...
          <p class="calcp-sci-text">${f(o||"Dosagem baseada em estudos clínicos controlados.")}</p>
        </div>
