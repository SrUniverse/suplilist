import{S as c}from"./stack-recommender-BUZ5Bm5D.js";import{S as d,s as l,A as p}from"./main-CLddz3nA.js";import{E as s}from"./evidence-D5RtUc7g.js";const v=()=>{const o=l.favorites;return Array.isArray(o)?o:[]},f=o=>{l.dispatch(p.REMOVE_FAVORITE,{supplementId:o})},g=[{key:"all",label:"Todos"},{key:"bulk",label:"Hipertrofia"},{key:"endurance",label:"Performance"},{key:"general",label:"Saúde Geral"},{key:"cut",label:"Emagrecimento"}];function u(o,e){return e==="all"?!0:(o.targets?.[e]??0)>0}function b(o,e){const a=[...o];if(e==="evidence"){const t={A:0,B:1,C:2};return a.sort((r,i)=>(t[r.evidenceLevel]??3)-(t[i.evidenceLevel]??3))}return e==="az"?a.sort((t,r)=>t.name.localeCompare(r.name)):a}class y{constructor(e){this.container=e,this._activeGoal="all",this._sortKey="evidence",this._handleStorageChange=this._onStorageChange.bind(this)}mount(){this._injectStyles(),window.removeEventListener("storage",this._handleStorageChange),window.addEventListener("storage",this._handleStorageChange),this._render()}unmount(){window.removeEventListener("storage",this._handleStorageChange),this.container.innerHTML=""}_onStorageChange(e){e.key===d.STATE&&this._render()}_getFavoriteSupplements(){return v().map(a=>c.find(t=>t.id===a)).filter(Boolean)}_injectStyles(){if(document.getElementById("fav-page-styles-v3"))return;const e=document.createElement("style");e.id="fav-page-styles-v3",e.textContent=`
      .fv-root {
        padding: 0 0 120px;
        min-height: 100%;
    ...
    `,document.head.appendChild(e)}_render(){const e=this._getFavoriteSupplements(),a=e.filter(r=>u(r,this._activeGoal)),t=b(a,this._sortKey);this.container.innerHTML=`
      <div class="fv-root">
        <div class="fv-header">
    ...
    `,this._bindEvents()}_renderCard(e){const a=e.image||`/assets/${e.id.replace(/-/g,"_")}.png`,t=s[e.evidenceLevel]??s.C,r=e.benefits?.[0]??"",i=e.pricePerGram?Math.round((e.dosage?.maintenance??5)*e.pricePerGram*30):null,n=i?`R$ ${i}`:null;return`
      <article class="fv-card" data-id="${e.id}">
        <div class="fv-card-img-wrap">
          ...
          />
          <button class="fv-card-fav-btn" data-action="remove-fav" data-id="${e.id}"
        ...
        <div class="fv-card-body">
          <div class="fv-card-badges">
            <span class="fv-badge-ev" style="background:${t.bg};color:${t.color};">
              NÍVEL ${e.evidenceLevel??"C"}
            </span>
            <span class="fv-badge-cat">${e.category??""}</span>
          </div>
          <h3 class="fv-card-name">${e.name}</h3>
          ${r?`<p class="fv-card-desc">${r}</p>`:""}
          ${n?`
            <div class="fv-card-price-row">
              <span class="fv-card-price-label">Melhor Preço</span>
              <span class="fv-card-price-val">${n}</span>
              <span class="fv-card-price-unit">/mês</span>
            </div>
          `:""}
          <div class="fv-card-actions">
            <button class="fv-btn-detail" data-action="go-list" data-id="${e.id}" type="button">
              Detalhes
            </button>
            <button class="fv-btn-buy" data-action="go-list" data-id="${e.id}" type="button">
              Ver Preços
            </button>
          </div>
        </div>
      </article>
    `}_bindEvents(){const e=this.container.querySelector(".fv-root");if(!e)return;e.addEventListener("click",t=>{const r=t.target.closest("[data-goal]");if(r){this._activeGoal=r.dataset.goal,this._render();return}const i=t.target.closest('[data-action="remove-fav"]');if(i){f(i.dataset.id),this._render();return}if(t.target.closest('[data-action="go-list"]')){window.history.pushState(null,null,"/list"),window.dispatchEvent(new PopStateEvent("popstate"));return}if(t.target.closest("#fv-btn-optimize")){window.history.pushState(null,null,"/my-stack"),window.dispatchEvent(new PopStateEvent("popstate"));return}if(t.target.closest("#fv-go-catalog")){window.history.pushState(null,null,"/list"),window.dispatchEvent(new PopStateEvent("popstate"));return}});const a=e.querySelector("#fv-sort");a&&a.addEventListener("change",t=>{this._sortKey=t.target.value,this._render()})}}export{y as default};
