import{s as _,t as j,o as z}from"./main-DL05SHUY.js";import{S as B}from"./stack-recommender-DY8xicr2.js";const N=["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"],D=c=>{if(!c)return"";const[o,i]=c.split("-");return`${N[parseInt(i,10)-1]} ${o}`},q=["Todos","Força & Performance","Proteínas","Vitaminas & Minerais","Adaptógenos & Foco","Cognição & Neuroproteção","Saúde Hormonal","Antioxidantes & Anti-inflamatórios","Sono & Recuperação","Saúde Geral"],F=()=>{const c={};for(const o of B)c[o.id]=o;return c},Q=(c,o)=>{let i=0;for(const a of c){const h=a.supplementId??a.id,r=o[h];if(r&&r.dosage&&r.pricePerGram){const u=r.dosage.maintenance||5,d=(r.dosage.unit||"g").toLowerCase();let n;if(d==="g")n=u;else if(d==="mg")n=u/1e3;else if(d==="mcg")n=u/1e6;else continue;i+=n*r.pricePerGram}}return i};class V{constructor(o){this.container=o,this._unsubscribe=null,this._searchQuery="",this._activeCategory="Todos",this._expandedCards=new Set}mount(){this._injectStyles(),this._render(),this._unsubscribe=_.subscribe(()=>this._render())}unmount(){this._unsubscribe?.()}_injectStyles(){if(document.getElementById("history-page-styles-v2"))return;const o=document.createElement("style");o.id="history-page-styles-v2",o.textContent=`
    ...
    `,document.head.appendChild(o)}_render(){const o=_.getState(),i=o.checkins||[],a=o.stack||[],h=F(),r=j(),u=30,d=new Set(i.map(t=>t.date).filter(Boolean)),n=d.size;let $=0;for(let t=0;t<u;t++)d.has(z(t))&&$++;const C=Math.round($/u*100),E=(Q(a,h)*n).toFixed(2).replace(".",","),L=["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"],S=[];for(let t=6;t>=0;t--){const e=z(t),s=new Date(e+"T12:00:00");S.push({iso:e,label:L[s.getDay()],isToday:t===0,hasCk:d.has(e),dayNum:s.getDate()})}const v={};for(const t of i){const e=t.supplementId||"unknown";v[e]||(v[e]=[]),v[e].push(t.date||"")}let l=Object.entries(v).map(([t,e])=>{const s=h[t],p=[...new Set(e)].filter(Boolean).sort(),g=p[0]||"",x=p[p.length-1]||"",m=p.length,w=g?Math.max(1,Math.ceil((new Date(r)-new Date(g))/864e5)+1):1,y=Math.min(w,30),k=Math.round(m/y*100);return{sid:t,name:s?.name||t,category:s?.category||"",image:s?.image||null,firstDate:g,lastDate:x,totalDays:m,totalPossible:y,adPct:k,dates:p.reverse(),lastCheckin:x}});l.sort((t,e)=>e.lastCheckin.localeCompare(t.lastCheckin));const b=this._searchQuery.toLowerCase().trim();if(b&&(l=l.filter(t=>t.name.toLowerCase().includes(b)||t.category.toLowerCase().includes(b))),this._activeCategory!=="Todos"){const t=this._activeCategory.toLowerCase();l=l.filter(e=>{const s=(e.category||"").toLowerCase();return s===t?!0:t==="saúde geral"?["saúde cardiovascular","saúde articular & pele","saúde intestinal","queima de gordura & recovery"].includes(s):t==="cognição & neuroproteção"?s==="energéticos & foco":!1})}const M=`
      <div class="hp-stats">
        <div class="hp-stat-card">
          ...
          <span class="hp-stat-value">${C}<span style="font-size:16px;font-weight:600;color:var(--color-text-muted)">%</span></span>
          <div class="hp-progress-bar">
            <div class="hp-progress-fill" style="width:${C}%"></div>
          </div>
        </div>
        <div class="hp-stat-card">
        ...
        <div class="hp-stat-card">
          ...
          <span class="hp-stat-value" style="font-size:20px;">R$ ${E}</span>
    ...
    `;let f;if(i.length===0)f=`
      ...
      `;else if(l.length===0)f=`
        <div class="hp-empty">
          <div class="hp-empty-icon">🔍</div>
          <div class="hp-empty-title">Nenhum resultado</div>
          <div class="hp-empty-sub">Tente outro nome ou categoria.</div>
        </div>
      `;else{const t=l.map(e=>{const s=this._expandedCards.has(e.sid),p=e.adPct>=80?"green":e.adPct>=60?"yellow":"red",g=e.firstDate?D(e.firstDate):"—",x=e.lastDate===r?"Presente":e.lastDate?D(e.lastDate):"—",m=e.image?`<img class="hp-sup-img" src="${e.image}" alt="${e.name}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">`:"",w=`<div class="hp-sup-img-placeholder" ${e.image?'style="display:none"':""}>💊</div>`,y=e.dates.map(k=>{const[H,T,A]=k.split("-");return`<div class="hp-log-date">${new Date(parseInt(H),parseInt(T)-1,parseInt(A)).toLocaleDateString("pt-BR",{weekday:"short",day:"numeric",month:"short",year:"numeric"})}</div>`}).join("");return`
            ...
            <div class="hp-sup-header" data-toggle="${e.sid}">
              ...
              <div class="hp-sup-info">
                ...
                <div class="hp-sup-meta">
                  ...
                  <span class="hp-sup-range">${g} → ${x}</span>
                </div>
              ...
              <button class="hp-expand-btn" data-toggle="${e.sid}">${s?"Fechar ▲":"Ver Logs ▼"}</button>
            </div>
            <div class="hp-logs-panel ${s?"open":""}" id="hp-logs-${e.sid}">
