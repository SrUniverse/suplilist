import{l as F,s as h,A as w}from"./main-CLddz3nA.js";import{S}from"./stack-recommender-BUZ5Bm5D.js";import{F as T}from"./vendor-CMLop-UK.js";import{e as f}from"./escape-Br5wU8qn.js";import{E as y}from"./evidence-D5RtUc7g.js";import{a as C}from"./affiliate-engine-YaoYSCQt.js";function A(a,t){const e=(t||"g").toLowerCase().trim();return e==="g"?a:e==="mg"?a/1e3:e==="mcg"?a/1e6:e==="ui"||e==="caps"||e==="capsules"||e==="pills"?a/1e3:a}const z=30,O=24,q=300,M=["Todos","Performance","Proteínas","Vitaminas","Adaptógenos","Hormônios","Cognição","Antioxidantes","Sono","Saúde Geral"],P=["Hipertrofia","Saúde Geral","Longevidade","Performance","Foco"],B={Hipertrofia:"bulk","Saúde Geral":"general",Longevidade:"general",Performance:"endurance",Foco:"endurance"},E=O;function I(a,t){const e=t&&t[a.id]?Object.values(t[a.id]):null;return!e||!e.length?null:e.reduce((r,n)=>r.price<n.price?r:n)}function j(a,t){const e=I(a,t);if(e)return{price:e.price,label:e.label};const r=a.dosage?.maintenance??5,n=a.dosage?.unit||"g",o=a.pricePerGram??.3;return{price:A(r,n)*o*z,label:null}}function R(a,t){const e=I(a,t);if(e)return`R$ ${(e.price/z).toFixed(2).replace(".",",")} / dose`;const r=a.dosage?.maintenance??5,n=a.dosage?.unit||"g",o=a.pricePerGram??.3;return`R$ ${(A(r,n)*o).toFixed(2).replace(".",",")} / dose`}function G(a,t){const e=a.id;if(!t||!t[e])return null;const r=Object.values(t[e]),n=Math.max(...r.map(o=>o.saving||0));return n>0?n:null}function N(a,t){if(!t||t==="Todos")return!0;const e=(a.category||"").toLowerCase();return t==="Performance"?e.includes("força")||e.includes("performance")||e.includes("resistência")||e.includes("endurance")||e.includes("queima")||e.includes("gordura")||e.includes("recovery"):t==="Proteínas"?e.startsWith("prote"):t==="Vitaminas"?e.includes("vitam"):t==="Adaptógenos"?e.includes("adapt"):t==="Hormônios"?e.includes("hormon")||e.includes("testoster")||e.includes("libido"):t==="Cognição"?e.includes("cogni")||e.includes("neuro")||e.includes("foco"):t==="Antioxidantes"?e.includes("antioxid")||e.includes("anti-inflamat"):t==="Sono"?e.includes("sono")||e.includes("recuper"):t==="Saúde Geral"?e.includes("saúde")||e.includes("geral")||e.includes("imun")||e.includes("intestin")||e.includes("articular")||e.includes("pele")||e.includes("mineral")||e.includes("miner")||e.includes("omega")||e.includes("ômega"):!0}function H(a,t){if(!t)return!0;const e=B[t];return e?a.targets&&a.targets[e]!=null&&a.targets[e]>0:!0}function k(){return new Set(h.favorites??[])}function D(a){k().has(a)?h.dispatch(w.REMOVE_FAVORITE,{supplementId:a}):h.dispatch(w.ADD_FAVORITE,{supplementId:a})}function $(a){return`R$ ${Number(a).toFixed(2).replace(".",",").replace(/\B(?=(\d{3})+(?!\d))/g,".")}`}class Q{constructor(t,e={}){this.container=t,this._unsubscribe=null,this._fuse=null,this._allItems=[],this._filtered=[],this._page=0,this._query="",this._category="Todos",this._objective=e.objective||"",this._prices=null,this._modalOpen=null,this._debounceTimer=null,this._observer=null,this._boundKeydown=this._onKeydown.bind(this),this._scrollLockStack=[]}mount(){this._attachStyles(),this._allItems=S,this._fuse=new T(this._allItems,{keys:["name","category","benefits"],threshold:.35,includeScore:!0,ignoreLocation:!0}),this._fetchController=new AbortController,fetch("/data/prices.json",{signal:this._fetchController.signal}).then(t=>t.ok?t.json():Promise.reject(new Error(`HTTP ${t.status}`))).then(t=>{this._prices=t,this._renderGrid()}).catch(t=>{t.name!=="AbortError"&&F.warn("[ListPage] prices.json failed to load, using estimates:",t.message)}),this._render(),this._syncObjectiveChip(),this._applyFilters(),this._renderStats(),this._renderGrid(),this._initInfiniteScroll(),this._attachListeners(),this._unsubscribe=h.subscribe(()=>this._refreshCardStates()),document.addEventListener("keydown",this._boundKeydown)}unmount(){this._fetchController?.abort(),this._unsubscribe?.(),this._observer?.disconnect(),document.removeEventListener("keydown",this._boundKeydown),this._closeModal(),clearTimeout(this._debounceTimer);const t=document.getElementById("router-outlet");t&&(t.style.overflow=""),document.body.style.overflow=""}_attachStyles(){if(document.getElementById("list-page-styles"))return;const t=document.createElement("style");t.id="list-page-styles",t.textContent=`
      /* ── Layout ── */
      #lp-root {
        ...
        <div id="lp-filters">
          <div class="lp-filter-row-wrap">
            <div class="lp-filter-row" id="lp-cat-row">
              ...
              ${M.map(t=>`<button class="lp-chip${t==="Todos"?" active":""}" data-cat="${t}">${t}</button>`).join("")}
            </div>
          </div>
          <div class="lp-filter-row-wrap">
            <div class="lp-filter-row" id="lp-obj-row">
              <span class="lp-filter-label">Objetivo</span>
              ${P.map(t=>`<button class="lp-chip" data-obj="${t}">${t}</button>`).join("")}
            </div>
          </div>
        </div>
    ...
    `}_renderStats(){const t=S.length,e=(h.stack||[]).length,r=k().size,n=S.filter(c=>c.evidenceLevel==="A").length,o=157.1,s=(c,g,v,b,x)=>{const d=this.container.querySelector(c),m=this.container.querySelector(g);if(!d||!m)return;m.textContent=v;const i=b>0?Math.min(1,v/b):0;d.style.stroke=x,d.style.strokeDashoffset=o*(1-i)};s("#lp-ring-total","#lp-stat-total",t,t,"var(--color-brand)"),s("#lp-ring-stack","#lp-stat-stack",e,t,"#8B5CF6"),s("#lp-ring-favs","#lp-stat-favs",r,t,"#EF4444"),s("#lp-ring-eva","#lp-stat-eva",n,t,"#22C55E"),this.container.querySelector("#lp-stat-stack")?.closest(".lp-stat-box")?.classList.toggle("stat--empty",e===0)}_applyFilters(){let t=this._allItems;this._query.trim()&&(t=this._fuse?this._fuse.search(this._query).map(r=>r.item):t.filter(r=>r.name.toLowerCase().includes(this._query.toLowerCase()))),t=t.filter(r=>N(r,this._category)),t=t.filter(r=>H(r,this._objective)),this._filtered=t;const e=this.container.querySelector("#lp-results-label");e&&(e.textContent=this._query||this._category!=="Todos"||this._objective?`${this._filtered.length} resultado(s)`:"")}_renderGrid(){if(this._modalOpen)return;const t=this.container.querySelector("#lp-grid");if(!t)return;if(this._page=0,t.innerHTML="",!this._filtered.length){t.innerHTML=`
        <div class="lp-empty">
