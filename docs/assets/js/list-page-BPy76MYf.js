import{s as x,l as M,A as E}from"./main-CqoYzm2q.js";import{S as L}from"./stack-recommender-HKiD6amT.js";import{F as j}from"./vendor-Vk1E4gpc.js";import{e as d}from"./escape-Br5wU8qn.js";import{E as k}from"./evidence-D5RtUc7g.js";import{a as I}from"./affiliate-engine-EKKcrKVa.js";import{C as R}from"./checkout-modal-BF3qKyld.js";function z(s,e){const t=(e||"g").toLowerCase().trim();return t==="g"?s:t==="mg"?s/1e3:t==="mcg"?s/1e6:t==="ui"||t==="caps"||t==="capsules"||t==="pills"?s/1e3:s}const A=30,B=24,G=300;class N{constructor(e,t=[],i,o={}){this.container=e,this.items=t,this.renderItem=i,this.itemHeight=o.itemHeight||80,this.bufferSize=o.bufferSize||5,this.scrollElement=o.scrollElement||window,this.columns=o.columns||1,this.gap=o.gap??12,this.visibleStartIndex=0,this.visibleEndIndex=0,this.containerHeight=0,this.scrollTop=0,this.listElement=null,this.itemElements=[],this._scrollHandler=this._onScroll.bind(this),this._resizeObserver=null,this._intersectionObserver=null}mount(){this._createContainer(),this._getContainerHeight(),this._render(),this._attachListeners()}unmount(){this._detachListeners(),this._cleanupObservers(),this.listElement&&this.listElement.remove()}updateItems(e){this.items=e,this.visibleStartIndex=0,this.visibleEndIndex=0,this._render()}scrollToIndex(e){const t=Math.floor(e/this.columns),i=this.itemHeight+this.gap,o=Math.max(0,t*i-this.containerHeight/2);this.scrollElement===window?window.scrollTo({top:o}):this.scrollElement.scrollTop=o}_createContainer(){this.listElement=document.createElement("div"),this.listElement.className="virtual-scroller-list",this.listElement.style.cssText=`
      position: relative;
      width: 100%;
      contain: layout style paint;
    `,this.container.appendChild(this.listElement)}_getContainerHeight(){this.scrollElement===window?this.containerHeight=window.innerHeight:this.containerHeight=this.scrollElement.clientHeight}_render(){this._getContainerHeight(),this._updateVisibleRange();const e=this.columns,t=this.gap,i=Math.ceil(this.items.length/e),o=this.itemHeight+t,r=i*o-t;this.listElement.style.height=r+"px";const p=[],h=this.visibleStartIndex,g=this.visibleEndIndex;for(let f=h;f<=g;f++){const y=f*o,l=f*e,c=Math.min(l+e,this.items.length);let n="";for(let u=l;u<c;u++){const a=this.items[u];a&&(n+=`<div class="virtual-col" style="flex:1;min-width:0;">${this.renderItem(a,u)}</div>`)}p.push(`
        <div class="virtual-item" data-row="${f}" style="
          position: absolute;
          top: ${y}px;
          left: 0; right: 0;
    ...
    `}_renderStats(){const e=L.length,t=(x.stack||[]).length,i=w().size,o=L.filter(g=>g.evidenceLevel==="A").length,r=157.1,p=(g,f,y,l,c)=>{const n=this.container.querySelector(g),u=this.container.querySelector(f);if(!n||!u)return;u.textContent=y;const a=l>0?Math.min(1,y/l):0;n.style.stroke=c,n.style.strokeDashoffset=r*(1-a)};p("#lp-ring-total","#lp-stat-total",e,e,"var(--color-brand)"),p("#lp-ring-stack","#lp-stat-stack",t,e,"#8B5CF6"),p("#lp-ring-favs","#lp-stat-favs",i,e,"#EF4444"),p("#lp-ring-eva","#lp-stat-eva",o,e,"#22C55E"),this.container.querySelector("#lp-stat-stack")?.closest(".lp-stat-box")?.classList.toggle("stat--empty",t===0)}_applyFilters(){let e=this._allItems;if(this._query.trim()&&(e=this._fuse?this._fuse.search(this._query).map(r=>r.item):e.filter(r=>r.name.toLowerCase().includes(this._query.toLowerCase()))),e=e.filter(r=>K(r,this._category)),e=e.filter(r=>Y(r,this._objective)),this._evidenceFilter&&(e=e.filter(r=>r.evidenceLevel===this._evidenceFilter)),this._maxPriceFilter<300&&(e=e.filter(r=>{const p=this._prices?.[r.id],h=p?parseFloat(p.shopee?.price||p.amazon?.price||p.mercado_livre?.price||0):0;return(h>0?h:r.estimatedMonthlyCost||0)<=this._maxPriceFilter})),this._benefitsFilter.size>0&&(e=e.filter(r=>{const p=(r.benefits||[]).map(h=>h.toLowerCase());return[...this._benefitsFilter].every(h=>{const g=h.toLowerCase();return p.some(f=>f.includes(g)||g.includes(f))})})),(x.getState().user?.tier??"free")==="free"&&e.length>3){const r=[...e];r.splice(3,0,{id:"sponsored-ad",isAd:!0}),this._filtered=r}else this._filtered=e;const o=this.container.querySelector("#lp-results-label");if(o){const r=this._evidenceFilter||this._maxPriceFilter<300||this._benefitsFilter.size>0;o.textContent=this._query||this._category!=="Todos"||this._objective||r?`${this._filtered.length} resultado(s)`:""}}_renderGrid(){if(this._modalOpen)return;const e=this.container.querySelector("#lp-grid");if(!e)return;if(!this._filtered.length){e.innerHTML=`
        ...
        </div>`,this._scroller&&(this._scroller.unmount(),this._scroller=null);return}this._scroller&&(this._scroller.unmount(),this._scroller=null),e.innerHTML="";const t=window.innerWidth,i=t>=1024?4:t>=640?3:2,o=t>=640?340:310;this._scroller=new N(e,this._filtered,r=>this._renderSupplementCard(r),{itemHeight:o,bufferSize:8,columns:i,gap:12}),this._scroller.mount(),this._page=1}_renderSupplementCard(e){if(e.isAd)return this._renderSponsoredAdCard();x.stack;const i=w().has(e.id),o=e.evidenceLevel,r=e.benefits?.[0]??"",p=e.image||`/assets/${e.id.replace(/-/g,"_")}.png`,h=T(e,this._prices),g=C(e,this._prices),f=H(e,this._prices),y=o?`lp-card-ev-badge--${String(o).toLowerCase()}`:"",l=o?`NÍVEL ${d(String(o))}`:"",c=h?`<span class="lp-card-savings">ECONOMIZE R$ ${d(String(h))}</span>`:"",n=o?`<span class="lp-card-ev-badge ${y}">${l}</span>`:"";return`
      <div class="lp-card" role="listitem" data-id="${e.id}">
        ...
        <div class="lp-card-info">
          ...
          <div class="lp-card-price-row">
            ...
            <span class="lp-card-dose">${f}</span>
          </div>
    ...
    `}_openCheckoutModal(){R.show({tier:"pro"})}_loadMore(){const e=this._page*P;if(e>=this._filtered.length)return;const t=this.container.querySelector("#lp-grid");if(!t)return;const i=this.container.querySelector("#lp-loading-more");i&&(i.style.display="block"),requestAnimationFrame(()=>{t.appendChild(this._buildFragment(e,e+P)),this._page++,i&&(i.style.display="none")})}_buildFragment(e,t){const i=document.createDocumentFragment();x.stack;const o=w();return this._filtered.slice(e,t).forEach(r=>{const p=o.has(r.id),h=r.evidenceLevel,g=k[h]??k.C,f=r.benefits?.[0]??"",y=r.image||`/assets/${r.id.replace(/-/g,"_")}.png`,l=T(r,this._prices),c=C(r,this._prices),n=H(r,this._prices);let u="";l?u=`<div class="lp-card-top-badge" style="background:rgba(34,197,94,0.10);color:#22C55E;">
        ...
        <div class="lp-card-img-wrap">
          ...
          />
          ${h?`<span class="lp-card-ev-badge" style="background:${g.bg};color:${g.color};">EV. ${d(String(h))}</span>`:""}
        </div>
        <div class="lp-card-info">
          <p class="lp-card-name">${d(r.name)}</p>
          <p class="lp-card-cat">${d(r.category??"")}</p>
          ${f?`<p class="lp-card-desc">${d(f)}</p>`:""}
          <div class="lp-card-price-row">
        ...
        <div class="lp-price-card${_?" lp-price-card--best":""}">
          ...
          <div style="display:flex;align-items:center;gap:8px;">
            ...
            <a class="lp-price-link"
               href="${O(D(b.url)?b.url:g[v])}"
               target="_blank"
               rel="noopener noreferrer"
               data-aff-id="${d(t.id)}"
               data-aff-mp="${d(v)}">Ver Oferta →</a>
          </div>
        </div>`}).join("")}else{const a=C(t,null);f=[{key:"amazon",label:"Amazon"},{key:"mercadolivre",label:"Mercado Livre"},{key:"shopee",label:"Shopee"}].map(({key:v,label:b})=>`
        ...
        <div class="lp-modal-top">
          ...
          <div class="lp-modal-info-col">
            <div>
              ...
              <div class="lp-price-cards">${f}</div>
            </div>
