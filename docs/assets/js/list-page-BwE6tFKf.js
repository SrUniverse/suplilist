import{l as T,s as y,A as E}from"./main-CJjCf5yQ.js";import{S as L}from"./stack-recommender-C1iG7okR.js";import{F as M}from"./vendor-BTvocYqE.js";import{e as n}from"./escape-Br5wU8qn.js";import{E as w}from"./evidence-D5RtUc7g.js";import{a as I}from"./affiliate-engine-DREwJ-iw.js";function F(a,e){const t=(e||"g").toLowerCase().trim();return t==="g"?a:t==="mg"?a/1e3:t==="mcg"?a/1e6:t==="ui"||t==="caps"||t==="capsules"||t==="pills"?a/1e3:a}const H=30,B=24,R=300;class G{constructor(e,t=[],r,i={}){this.container=e,this.items=t,this.renderItem=r,this.itemHeight=i.itemHeight||80,this.bufferSize=i.bufferSize||5,this.scrollElement=i.scrollElement||window,this.visibleStartIndex=0,this.visibleEndIndex=0,this.containerHeight=0,this.scrollTop=0,this.listElement=null,this.itemElements=[],this._scrollHandler=this._onScroll.bind(this),this._resizeObserver=null,this._intersectionObserver=null}mount(){this._createContainer(),this._getContainerHeight(),this._render(),this._attachListeners()}unmount(){this._detachListeners(),this._cleanupObservers(),this.listElement&&this.listElement.remove()}updateItems(e){this.items=e,this.visibleStartIndex=0,this.visibleEndIndex=0,this._render()}scrollToIndex(e){const t=Math.max(0,e*this.itemHeight-this.containerHeight/2);this.scrollElement.scrollTop=t}_createContainer(){this.listElement=document.createElement("div"),this.listElement.className="virtual-scroller-list",this.listElement.style.cssText=`
      position: relative;
      width: 100%;
      contain: layout style paint;
    `,this.container.appendChild(this.listElement)}_getContainerHeight(){this.scrollElement===window?this.containerHeight=window.innerHeight:this.containerHeight=this.scrollElement.clientHeight}_render(){this._getContainerHeight(),this._updateVisibleRange();const e=this.items.length*this.itemHeight;this.listElement.style.height=e+"px";const t=[];for(let r=this.visibleStartIndex;r<=this.visibleEndIndex;r++){const i=this.items[r];if(i){const s=r*this.itemHeight,p=this.renderItem(i,r);t.push(`
    ...
    `}_renderStats(){const e=L.length,t=(y.stack||[]).length,r=k().size,i=L.filter(f=>f.evidenceLevel==="A").length,s=157.1,p=(f,m,x,c,d)=>{const l=this.container.querySelector(f),h=this.container.querySelector(m);if(!l||!h)return;h.textContent=x;const o=c>0?Math.min(1,x/c):0;l.style.stroke=d,l.style.strokeDashoffset=s*(1-o)};p("#lp-ring-total","#lp-stat-total",e,e,"var(--color-brand)"),p("#lp-ring-stack","#lp-stat-stack",t,e,"#8B5CF6"),p("#lp-ring-favs","#lp-stat-favs",r,e,"#EF4444"),p("#lp-ring-eva","#lp-stat-eva",i,e,"#22C55E"),this.container.querySelector("#lp-stat-stack")?.closest(".lp-stat-box")?.classList.toggle("stat--empty",t===0)}_applyFilters(){let e=this._allItems;this._query.trim()&&(e=this._fuse?this._fuse.search(this._query).map(r=>r.item):e.filter(r=>r.name.toLowerCase().includes(this._query.toLowerCase()))),e=e.filter(r=>J(r,this._category)),e=e.filter(r=>K(r,this._objective)),this._evidenceFilter&&(e=e.filter(r=>r.evidenceLevel===this._evidenceFilter)),this._maxPriceFilter<300&&(e=e.filter(r=>{const i=this._prices?.[r.id],s=i?parseFloat(i.shopee?.price||i.amazon?.price||i.mercado_livre?.price||0):0;return(s>0?s:r.estimatedMonthlyCost||0)<=this._maxPriceFilter})),this._benefitsFilter.size>0&&(e=e.filter(r=>{const i=(r.benefits||[]).map(s=>s.toLowerCase());return[...this._benefitsFilter].every(s=>{const p=s.toLowerCase();return i.some(v=>v.includes(p)||p.includes(v))})})),this._filtered=e;const t=this.container.querySelector("#lp-results-label");if(t){const r=this._evidenceFilter||this._maxPriceFilter<300||this._benefitsFilter.size>0;t.textContent=this._query||this._category!=="Todos"||this._objective||r?`${this._filtered.length} resultado(s)`:""}}_renderGrid(){if(this._modalOpen)return;const e=this.container.querySelector("#lp-grid");if(e){if(!this._filtered.length){e.innerHTML=`
        <div class="lp-empty">
        ...
        </div>`,this._scroller&&(this._scroller.unmount(),this._scroller=null);return}this._scroller&&(this._scroller.unmount(),this._scroller=null),e.innerHTML="",this._scroller=new G(e,this._filtered,t=>this._renderSupplementCard(t),{itemHeight:310,bufferSize:5}),this._scroller.mount(),this._page=1}}_renderSupplementCard(e){y.stack;const r=k().has(e.id),i=e.evidenceLevel,s=w[i]??w.C,p=e.benefits?.[0]??"",v=e.image||`/assets/${e.id.replace(/-/g,"_")}.png`,f=A(e,this._prices),m=C(e,this._prices),x=q(e,this._prices);let c="";return f?c=`<div class="lp-card-top-badge" style="background:rgba(34,197,94,0.10);color:#22C55E;">
        ECONOMIZE R$ ${n(String(f))} NA AMAZON
      </div>`:e.category&&(c=`<div class="lp-card-top-badge" style="background:var(--color-brand-muted);color:var(--color-brand);">
        ${n(e.category)}
      </div>`),`
      <div class="lp-card" role="listitem" data-id="${e.id}">
        ...
        <div class="lp-card-img-wrap">
          ...
          />
          ${i?`<span class="lp-card-ev-badge" style="background:${s.bg};color:${s.color};">EV. ${n(String(i))}</span>`:""}
        </div>
        <div class="lp-card-info">
          ...
          <div class="lp-card-price-row">
            <span class="lp-card-price">${S(m.price)}</span>
            <span class="lp-card-dose">${x}</span>
          </div>
    ...
    `}_loadMore(){const e=this._page*P;if(e>=this._filtered.length)return;const t=this.container.querySelector("#lp-grid");if(!t)return;const r=this.container.querySelector("#lp-loading-more");r&&(r.style.display="block"),requestAnimationFrame(()=>{t.appendChild(this._buildFragment(e,e+P)),this._page++,r&&(r.style.display="none")})}_buildFragment(e,t){const r=document.createDocumentFragment();y.stack;const i=k();return this._filtered.slice(e,t).forEach(s=>{const p=i.has(s.id),v=s.evidenceLevel,f=w[v]??w.C,m=s.benefits?.[0]??"",x=s.image||`/assets/${s.id.replace(/-/g,"_")}.png`,c=A(s,this._prices),d=C(s,this._prices),l=q(s,this._prices);let h="";c?h=`<div class="lp-card-top-badge" style="background:rgba(34,197,94,0.10);color:#22C55E;">
          ECONOMIZE R$ ${n(String(c))} NA AMAZON
        </div>`:s.category&&(h=`<div class="lp-card-top-badge" style="background:var(--color-brand-muted);color:var(--color-brand);">
          ${n(s.category)}
        </div>`);const o=document.createElement("div");o.className="lp-card",o.role="listitem",o.dataset.id=s.id,o.innerHTML=`
        ${h}
        <div class="lp-card-img-wrap">
          <img class="lp-card-img"
            src="${n(x)}"
            ...
            onerror="this.style.display='none'"
          />
          ${v?`<span class="lp-card-ev-badge" style="background:${f.bg};color:${f.color};">EV. ${n(String(v))}</span>`:""}
        </div>
        <div class="lp-card-info">
          <p class="lp-card-name">${n(s.name)}</p>
          <p class="lp-card-cat">${n(s.category??"")}</p>
          ${m?`<p class="lp-card-desc">${n(m)}</p>`:""}
          <div class="lp-card-price-row">
            <span class="lp-card-price">${S(d.price)}</span>
            <span class="lp-card-dose">${l}</span>
          </div>
          <div class="lp-card-actions">
            <button class="lp-btn-fav${p?" faved":""}" data-action="toggle-fav" data-id="${s.id}" aria-label="${p?"Remover dos favoritos":"Favoritar"}" type="button">
              ${p?"♥":"♡"}
            </button>
            <button class="lp-btn-ver-precos" data-action="open-modal" data-id="${s.id}" type="button">
              VER PREÇOS →
            </button>
          </div>
        </div>
      `,r.appendChild(o)}),r}_initInfiniteScroll(){const e=this.container.querySelector("#lp-sentinel");if(!(!e||!("IntersectionObserver"in window)))try{this._observer=new IntersectionObserver(t=>{t[0].isIntersecting&&this._loadMore()},{rootMargin:"300px"}),this._observer.observe(e)}catch{this._observer?.disconnect(),this._observer=null}}_refreshCardStates(){this._renderStats();const e=y.stack??[],t=k();if(this.container.querySelectorAll(".lp-card").forEach(r=>{const i=r.dataset.id,s=t.has(i),p=r.querySelector('[data-action="toggle-fav"]');p&&(p.classList.toggle("faved",s),p.textContent=s?"♥":"♡",p.setAttribute("aria-label",s?"Remover dos favoritos":"Favoritar"))}),this._modalOpen){const r=document.querySelector("#lp-modal-add-btn");if(r){const i=e.some(s=>s.supplementId===this._modalOpen);r.classList.toggle("in-stack",i),r.textContent=i?"✓ Já no Stack":"+ Adicionar ao Stack"}}}_pushScrollLock(e="modal"){this._scrollLockStack.includes(e)||(this._scrollLockStack.push(e),document.body.classList.add("has-modal-open"))}_popScrollLock(e="modal"){const t=this._scrollLockStack.indexOf(e);t!==-1&&(this._scrollLockStack.splice(t,1),this._scrollLockStack.length===0&&document.body.classList.remove("has-modal-open"))}_openModal(e){this._closeModal();const t=this._allItems.find(o=>o.id===e);if(!t)return;this._modalOpen=e;const r=t.evidenceLevel,i=w[r]??w.C,s=t.image||`/assets/${t.id.replace(/-/g,"_")}.png`,v=(y.stack??[]).some(o=>o.supplementId===t.id),f=I.getLinks(t.name,t.id);let m="";const x=t.id;if(this._prices&&this._prices[x]){const o=this._prices[x],b=Object.entries(o).reduce((u,[g,_])=>$(_)<$(o[u])?g:u,Object.keys(o)[0]);m=Object.entries(o).map(([u,g])=>{const _=u===b,z=g.qty&&g.unit?`${g.qty}${g.unit} · R$ ${(g.pricePerUnit??g.price).toFixed(2).replace(".",",")}/${g.unit}`:"";return`
        <div class="lp-price-card${_?" lp-price-card--best":""}">
          <div class="lp-price-card-left">
            <div class="lp-price-card-store-row">
              <span class="lp-price-card-store">${n(String(g.label??""))}</span>
              ${_?'<span class="lp-price-best-badge">✓ Melhor custo-benefício</span>':""}
            </div>
            <span class="lp-price-card-val">${S(g.price)}</span>
            ${z?`<span class="lp-price-qty">${n(z)}</span>`:""}
          </div>
          <div style="display:flex;align-items:center;gap:8px;">
            ${g.saving?`<span class="lp-price-saving">-R$ ${n(String(g.saving))}</span>`:""}
            <a class="lp-price-link"
               href="${O(N(g.url)?g.url:f[u])}"
               target="_blank"
               rel="noopener noreferrer"
               data-aff-id="${n(t.id)}"
               data-aff-mp="${n(u)}">Ver Oferta →</a>
          </div>
        </div>`}).join("")}else{const o=C(t,null);m=[{key:"amazon",label:"Amazon"},{key:"mercadolivre",label:"Mercado Livre"},{key:"shopee",label:"Shopee"}].map(({key:u,label:g})=>`
        <div class="lp-price-card">
          <div class="lp-price-card-left">
            <span class="lp-price-card-store">${n(g)}</span>
            <span class="lp-price-card-val">${S(o.price)}</span>
          </div>
          <a class="lp-price-link"
             href="${O(f[u])}"
             target="_blank"
      ...
      `).join("")}const c=t.warnings?.length?`<ul>${t.warnings.map(o=>`<li>${n(o)}</li>`).join("")}</ul>`:'<p style="color:var(--color-text-muted)">Nenhum aviso registrado.</p>',d=t.sideEffects?.length?`<p style="font-weight:600;color:var(--color-text-secondary);margin:10px 0 4px;">Efeitos Colaterais</p><ul>${t.sideEffects.map(o=>`<li>${n(o)}</li>`).join("")}</ul>`:"",l=document.createElement("div");l.id="lp-modal-overlay",l.innerHTML=`
      <div id="lp-modal-box" role="dialog" aria-modal="true" aria-label="${t.name}">
        ...
        <div class="lp-modal-top">
          <div class="lp-modal-img-col">
            <div class="lp-modal-img-wrap">
              <img class="lp-modal-img" src="${s}" alt="${n(t.name)}" onerror="this.style.display='none'" />
            </div>
          ...
          <div class="lp-modal-info-col">
            <div>
              ...
              <div class="lp-price-cards">${m}</div>
            </div>
            ...
            <div>
              ...
              <div class="lp-tab-content">
                ...
                <div class="lp-tab-pane" id="lp-tab-benefits">
                  ${t.benefits?.length?`<ul>${t.benefits.map(o=>`<li>${n(o)}</li>`).join("")}</ul>`:'<p style="color:var(--color-text-muted)">Sem dados.</p>'}
                </div>
        ...
        <div class="lp-modal-bottom">
          <button id="lp-modal-add-btn" class="lp-modal-add-btn${v?" in-stack":""}" data-id="${t.id}">
            ${v?"✓ Já no Stack":"+ Adicionar ao Stack"}
          </button>
        </div>
      </div>
    `,document.body.appendChild(l),this._pushScrollLock("modal"),l.querySelectorAll(".lp-tab").forEach(o=>{o.addEventListener("click",()=>{l.querySelectorAll(".lp-tab").forEach(u=>u.classList.remove("active")),l.querySelectorAll(".lp-tab-pane").forEach(u=>u.classList.remove("active")),o.classList.add("active");const b=l.querySelector(`#lp-tab-${o.dataset.tab}`);b&&b.classList.add("active")})}),l.querySelector("#lp-modal-close").addEventListener("click",()=>this._closeModal()),l.addEventListener("click",o=>{o.target===l&&this._closeModal()}),l.addEventListener("click",o=>{const b=o.target.closest("[data-aff-mp]");b&&I.trackClick(b.dataset.affId,b.dataset.affMp)});const h=l.querySelector("#lp-modal-add-btn");h.addEventListener("click",o=>{o.stopPropagation();const b=h.dataset.id,u=this._allItems.find(_=>_.id===b);if(!u)return;(y.stack??[]).some(_=>_.supplementId===b)?y.dispatch(E.REMOVE_FROM_STACK,{supplementId:b}):y.dispatch(E.ADD_TO_STACK,{supplementId:u.id,name:u.name,dosage:u.dosage?.maintenance??5,unit:u.dosage?.unit??"g",quantity:0}),this._refreshCardStates()})}_closeModal(){const e=document.getElementById("lp-modal-overlay");e&&(e.remove(),this._popScrollLock("modal")),this._modalOpen=null}_onKeydown(e){e.key==="Escape"&&this._modalOpen&&this._closeModal()}_syncObjectiveChip(){if(!this._objective)return;const e=this.container.querySelector("#lp-obj-row");if(!e)return;const t=e.querySelector(`[data-obj="${this._objective}"]`);t&&t.classList.add("active")}_attachListeners(){const e=this.container.querySelector("#lp-search");e&&e.addEventListener("input",c=>{clearTimeout(this._debounceTimer),this._debounceTimer=setTimeout(()=>{this._debounceTimer=null,this._query=c.target.value,this._applyFilters(),this._renderGrid(),this._saveSearchHistory(this._query)},R)});const t=this.container.querySelector("#lp-trending");t&&t.addEventListener("click",c=>{const d=c.target.closest("[data-trend]");if(!d)return;const l=this.container.querySelector("#lp-search");l&&(l.value=d.dataset.trend),this._query=d.dataset.trend,this._applyFilters(),this._renderGrid(),this._saveSearchHistory(this._query)});const r=this.container.querySelector("#lp-adv-filters-btn"),i=this.container.querySelector("#lp-advanced-panel");r&&i&&r.addEventListener("click",()=>{this._advancedPanelOpen=!this._advancedPanelOpen,i.style.display=this._advancedPanelOpen?"flex":"none",r.classList.toggle("active",this._advancedPanelOpen)});const s=this.container.querySelector("#lp-price-range"),p=this.container.querySelector("#lp-price-range-val");s&&p&&s.addEventListener("input",c=>{const d=parseInt(c.target.value,10);this._maxPriceFilter=d,p.textContent=d>=300?"R$ 300 (Qualquer preço)":`R$ ${d}`,this._applyFilters(),this._renderGrid()}),i&&i.addEventListener("click",c=>{const d=c.target.closest(".lp-evidence-filter");if(d){const h=d.dataset.evidence,o=d.classList.contains("active");i.querySelectorAll(".lp-evidence-filter").forEach(b=>b.classList.remove("active")),o?this._evidenceFilter="":(d.classList.add("active"),this._evidenceFilter=h),this._applyFilters(),this._renderGrid();return}const l=c.target.closest(".lp-benefit-filter");if(l){const h=l.dataset.benefit;this._benefitsFilter.has(h)?(this._benefitsFilter.delete(h),l.classList.remove("active")):(this._benefitsFilter.add(h),l.classList.add("active")),this._applyFilters(),this._renderGrid();return}});const v=this.container.querySelector("#lp-history-panel");v&&v.addEventListener("click",c=>{const d=c.target.closest(".lp-history-chip");if(d){const h=d.dataset.query,o=this.container.querySelector("#lp-search");o&&(o.value=h),this._query=h,this._applyFilters(),this._renderGrid();return}if(c.target.closest("#lp-clear-history-btn")){localStorage.removeItem("suplilist:search-history"),this._renderSearchHistory();return}});const f=this.container.querySelector("#lp-cat-row");f&&f.addEventListener("click",c=>{const d=c.target.closest("[data-cat]");d&&(f.querySelectorAll(".lp-chip").forEach(l=>l.classList.remove("active")),d.classList.add("active"),this._category=d.dataset.cat,this._applyFilters(),this._renderGrid())});const m=this.container.querySelector("#lp-obj-row");m&&m.addEventListener("click",c=>{const d=c.target.closest("[data-obj]");if(!d)return;const l=d.classList.contains("active");m.querySelectorAll(".lp-chip").forEach(h=>h.classList.remove("active")),l?this._objective="":(d.classList.add("active"),this._objective=d.dataset.obj),this._applyFilters(),this._renderGrid()});const x=this.container.querySelector("#lp-grid");x&&x.addEventListener("click",c=>{const d=c.target.closest('[data-action="toggle-fav"]');if(d){c.stopPropagation(),Y(d.dataset.id),this._refreshCardStates();return}const l=c.target.closest('[data-action="open-modal"]');if(l){c.stopPropagation(),this._openModal(l.dataset.id);return}const h=c.target.closest(".lp-card");h&&h.dataset.id&&this._openModal(h.dataset.id)})}_getSearchHistory(){try{return JSON.parse(localStorage.getItem("suplilist:search-history")||"[]")}catch{return[]}}_saveSearchHistory(e){if(!e||!e.trim())return;const t=e.trim();let r=this._getSearchHistory();r=r.filter(i=>i.toLowerCase()!==t.toLowerCase()),r.unshift(t),localStorage.setItem("suplilist:search-history",JSON.stringify(r.slice(0,10))),this._renderSearchHistory()}_renderSearchHistory(){const e=this.container.querySelector("#lp-history-panel"),t=this.container.querySelector("#lp-history-chips");if(!e||!t)return;const r=this._getSearchHistory();if(r.length===0){e.style.display="none";return}e.style.display="flex",t.innerHTML=r.map(i=>`
      <button class="lp-chip lp-history-chip" data-query="${n(i)}" style="margin: 0; padding: 4px 10px; font-size: 11px; background: var(--color-surface-secondary); display: flex; align-items: center; gap: 4px; border-radius: 8px;">
        <span>🔍</span> ${n(i)}
      </button>
    `).join("")}}export{re as default};
