import{s as u,A as y,S as L}from"./main-Q5JJQYYZ.js";import{S as E}from"./stack-recommender-b07295YU.js";import{F as z}from"./vendor-CMLop-UK.js";import{e as m}from"./escape-Br5wU8qn.js";import{E as k}from"./evidence-D5RtUc7g.js";const A=["Todos","Adaptógeno","Proteínas","Energéticos","Vitaminas","Minerais","Saúde Geral"],I=["Hipertrofia","Saúde Geral","Longevidade","Performance","Foco"],O={Hipertrofia:"bulk","Saúde Geral":"general",Longevidade:"general",Performance:"endurance",Foco:"endurance"},S=24;function C(i,e){const t=i.id;if(e&&e[t]){const o=Object.values(e[t]).reduce((s,c)=>s.price<c.price?s:c);return{price:o.price,label:o.label}}const r=i.dosage?.maintenance??5,l=i.pricePerGram??.3;return{price:r*l*30,label:null}}function T(i,e){const t=i.id;if(e&&e[t]){const a=Object.values(e[t]).reduce((s,c)=>s.price<c.price?s:c);return i.dosage?.maintenance,`R$ ${(a.price/30).toFixed(2).replace(".",",")} / dose`}const r=i.dosage?.maintenance??5,l=i.pricePerGram??.3;return`R$ ${(r*l).toFixed(2).replace(".",",")} / dose`}function j(i,e){const t=i.id;if(!e||!e[t])return null;const r=Object.values(e[t]),l=Math.max(...r.map(p=>p.saving||0));return l>0?l:null}function q(i,e){if(!e||e==="Todos")return!0;const t=(i.category||"").toLowerCase(),r=e.toLowerCase();return r==="adaptógeno"?t.includes("adapt")||t.includes("erva"):r==="proteínas"?t.includes("prote"):r==="energéticos"?t.includes("energ")||t.includes("foco")||t.includes("estimul"):r==="vitaminas"?t.includes("vitam"):r==="minerais"?t.includes("miner")||t.includes("magnés")||t.includes("zinc")||t.includes("ferro"):r==="saúde geral"?t.includes("saúde")||t.includes("geral")||t.includes("imun")||t.includes("omega")||t.includes("ômega"):!0}function F(i,e){if(!e)return!0;const t=O[e];return t?i.targets&&i.targets[t]!=null&&i.targets[t]>0:!0}function w(){const i=u.favorites;if(Array.isArray(i))return new Set(i);try{const e=localStorage.getItem(L.FAVORITES);return e?new Set(JSON.parse(e)):new Set}catch{return new Set}}function M(i){w().has(i)?u.dispatch(y.REMOVE_FAVORITE,{supplementId:i}):u.dispatch(y.ADD_FAVORITE,{supplementId:i})}function $(i){return`R$ ${Number(i).toFixed(2).replace(".",",").replace(/\B(?=(\d{3})+(?!\d))/g,".")}`}class V{constructor(e){this.container=e,this._unsubscribe=null,this._fuse=null,this._allItems=[],this._filtered=[],this._page=0,this._query="",this._category="Todos",this._objective="",this._prices=null,this._modalOpen=null,this._debounceTimer=null,this._observer=null,this._boundKeydown=this._onKeydown.bind(this)}mount(){this._attachStyles(),this._allItems=E,this._fuse=new z(this._allItems,{keys:["name","category","benefits"],threshold:.35,includeScore:!0,ignoreLocation:!0}),fetch("/data/prices.json").then(e=>e.ok?e.json():null).then(e=>{this._prices=e,this._renderGrid()}).catch(()=>{}),this._render(),this._applyFilters(),this._renderStats(),this._renderGrid(),this._initInfiniteScroll(),this._attachListeners(),this._unsubscribe=u.subscribe(()=>this._refreshCardStates()),document.addEventListener("keydown",this._boundKeydown)}unmount(){this._unsubscribe?.(),this._observer?.disconnect(),document.removeEventListener("keydown",this._boundKeydown),this._closeModal(),clearTimeout(this._debounceTimer),document.body.style.overflow=""}_attachStyles(){if(document.getElementById("list-page-styles"))return;const e=document.createElement("style");e.id="list-page-styles",e.textContent=`
      /* ── Layout ── */
      #lp-root {
      ...
      <div id="lp-root">
        ...
        <div id="lp-filters">
          <div class="lp-filter-row" id="lp-cat-row">
            ...
            ${A.map(e=>`<button class="lp-chip${e==="Todos"?" active":""}" data-cat="${e}">${e}</button>`).join("")}
          </div>
          <div class="lp-filter-row" id="lp-obj-row">
            <span class="lp-filter-label">Objetivo</span>
            ${I.map(e=>`<button class="lp-chip" data-obj="${e}">${e}</button>`).join("")}
          </div>
        </div>
    ...
    `}_renderStats(){const e=this.container.querySelector("#lp-stat-total"),t=this.container.querySelector("#lp-stat-stack"),r=this.container.querySelector("#lp-stat-favs");e&&(e.textContent=E.length),t&&(t.textContent=(u.stack||[]).length),r&&(r.textContent=w().size)}_applyFilters(){let e=this._allItems;this._query.trim()&&(e=this._fuse?this._fuse.search(this._query).map(r=>r.item):e.filter(r=>r.name.toLowerCase().includes(this._query.toLowerCase()))),e=e.filter(r=>q(r,this._category)),e=e.filter(r=>F(r,this._objective)),this._filtered=e;const t=this.container.querySelector("#lp-results-label");t&&(t.textContent=this._query||this._category!=="Todos"||this._objective?`${this._filtered.length} resultado(s)`:"")}_renderGrid(){const e=this.container.querySelector("#lp-grid");if(!e)return;if(this._page=0,e.innerHTML="",!this._filtered.length){e.innerHTML=`
        ...
        </div>`;return}const t=this._buildFragment(0,S);e.appendChild(t),this._page=1}_loadMore(){const e=this._page*S;if(e>=this._filtered.length)return;const t=this.container.querySelector("#lp-grid");if(!t)return;const r=this.container.querySelector("#lp-loading-more");r&&(r.style.display="block"),requestAnimationFrame(()=>{t.appendChild(this._buildFragment(e,e+S)),this._page++,r&&(r.style.display="none")})}_buildFragment(e,t){const r=document.createDocumentFragment(),l=u.stack??[],p=w();return this._filtered.slice(e,t).forEach(a=>{const o=l.some(_=>_.supplementId===a.id),s=p.has(a.id),c=a.evidenceLevel,h=k[c]??k.C,b=a.benefits?.[0]??"",g=a.id.replace(/-/g,"_"),v=j(a,this._prices),n=C(a,this._prices),d=T(a,this._prices);let f="";v?f=`<div class="lp-card-top-badge" style="background:rgba(34,197,94,0.10);color:#22C55E;">
          ECONOMIZE R$ ${v} NA AMAZON
        </div>`:a.category&&(f=`<div class="lp-card-top-badge" style="background:var(--color-brand-muted);color:var(--color-brand);">
          ${a.category}
        </div>`);const x=document.createElement("div");x.className="lp-card",x.role="listitem",x.dataset.id=a.id,x.innerHTML=`
        ${f}
        <div class="lp-card-img-wrap">
          ...
          />
          ${c?`<span class="lp-card-ev-badge" style="background:${h.bg};color:${h.color};">EV. ${c}</span>`:""}
        </div>
        <div class="lp-card-info">
          <p class="lp-card-name">${m(a.name)}</p>
          <p class="lp-card-cat">${m(a.category??"")}</p>
          ${b?`<p class="lp-card-desc">${m(b)}</p>`:""}
          <div class="lp-card-price-row">
            <span class="lp-card-price">${$(n.price)}</span>
      ...
      `).join("")}else{const n=C(t,null);s=["Amazon","Mercado Livre","Shopee"].map(d=>`
        <div class="lp-price-card">
          <div class="lp-price-card-left">
            <span class="lp-price-card-store">${d}</span>
            <span class="lp-price-card-val">${$(n.price)}</span>
          </div>
          <a class="lp-price-link" href="#" target="_blank">Ver Oferta →</a>
        </div>
      `).join("")}const h=t.warnings?.length?`<ul>${t.warnings.map(n=>`<li>${n}</li>`).join("")}</ul>`:'<p style="color:var(--color-text-muted)">Nenhum aviso registrado.</p>',b=t.sideEffects?.length?`<p style="font-weight:600;color:var(--color-text-secondary);margin:10px 0 4px;">Efeitos Colaterais</p><ul>${t.sideEffects.map(n=>`<li>${n}</li>`).join("")}</ul>`:"",g=document.createElement("div");g.id="lp-modal-overlay",g.innerHTML=`
        ...
        <div class="lp-modal-top">
          <div class="lp-modal-img-col">
            <div class="lp-modal-img-wrap">
              <img class="lp-modal-img" src="/assets/${p}.png" alt="${m(t.name)}" onerror="this.style.display='none'" />
            </div>
            <p class="lp-modal-img-col-name">${m(t.name)}</p>
            <p class="lp-modal-img-col-cat">${m(t.category??"")}</p>
            ...
            <p style="font-size:13px;color:var(--color-text-secondary);line-height:1.5;margin:4px 0 0;">${m(t.benefits?.join(" · ")??"")}</p>
          </div>
            ...
            <div>
              ...
              <div class="lp-tab-content">
                <div class="lp-tab-pane active" id="lp-tab-dose">
                  <p style="margin:0 0 6px;"><strong style="color:var(--color-text-primary);">Dose de manutenção:</strong> ${m(String(t.dosage?.maintenance??"—"))} ${m(t.dosage?.unit??"")}</p>
                  <p style="margin:0 0 6px;"><strong style="color:var(--color-text-primary);">Limite superior:</strong> ${m(String(t.dosage?.upperLimit??"—"))} ${m(t.dosage?.unit??"")}</p>
                  <p style="margin:0;"><strong style="color:var(--color-text-primary);">Quando tomar:</strong> ${m(t.dosage?.timing??"—")}</p>
                </div>
                <div class="lp-tab-pane" id="lp-tab-benefits">
                  ${t.benefits?.length?`<ul>${t.benefits.map(n=>`<li>${m(n)}</li>`).join("")}</ul>`:'<p style="color:var(--color-text-muted)">Sem dados.</p>'}
                </div>
