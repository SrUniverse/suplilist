import{s as p,A as b,o as E}from"./main-Cc1NkQ61.js";import{S as x}from"./stack-recommender-DF0wTc9D.js";import{r as $}from"./evidence-D5RtUc7g.js";import{e as f}from"./escape-Br5wU8qn.js";import{a as k}from"./affiliate-engine-CoPw7yg1.js";function u(i){return i==null?null:i.supplementId??i.id??null}let g=null;async function S(){if(g)return g;try{g=await(await fetch("/data/prices.json")).json()}catch{g={}}return g}function y(i){return"R$ "+i.toFixed(2).replace(".",",")}function w(i){return i.reduce((e,t)=>{const o=x.find(d=>d.id===u(t))?.pricePerGram??0,r=parseFloat(t.dosage)||0,n=(t.unit||"g").toLowerCase();let l;if(n==="g")l=r;else if(n==="mg")l=r/1e3;else if(n==="mcg")l=r/1e6;else return e;return e+l*o*30},0)}function I(i){if(!i.length)return"0%";const e=[];for(let r=0;r<7;r++)e.push(E(r));const t=p.checkins??[],s=new Set(t.map(r=>r.date)),o=e.filter(r=>s.has(r)).length;return Math.round(o/7*100)+"%"}function z(i){const e=x.find(s=>s.id===u(i));return e?.image?e.image:`/assets/${(i.name??"").toLowerCase().replace(/\s+/g,"_").replace(/[^a-z0-9_]/g,"")}.png`}function C(i){return x.find(t=>t.id===u(i))?.evidenceLevel??"C"}function L(i){const e=parseFloat(i.quantity),t=parseFloat(i.dosage);return!e||!t||t<=0?null:Math.max(0,Math.floor(e/t))}const A=`
  /* Layout */
  .msp-wrap {
    display: flex;
...
`;class R{constructor(e){this.container=e,this._unsub=null,this._editId=null,this._modalOpen=!1,this._prices=null,this._docClickHandler=null}mount(){this._attachStyles(),this._render(),S().then(e=>{this._prices=e,this._renderReplenishment()}),this._unsub=p.subscribe((e,t)=>{(!t||["ADD_TO_STACK","REMOVE_FROM_STACK","UPDATE_STACK_ITEM","SET_STACK_QUANTITY","ADD_CHECKIN"].includes(t.type))&&this._renderAll()})}unmount(){this._docClickHandler&&(document.removeEventListener("click",this._docClickHandler),this._docClickHandler=null),this._unsub?.(),this._closeModal()}_attachStyles(){if(document.getElementById("msp2-styles"))return;const e=document.createElement("style");e.id="msp2-styles",e.textContent=A,document.head.appendChild(e)}_render(){this.container.innerHTML=`
    ...
    `,this._renderAll(),this._attachDelegatedListeners(),this.container.querySelector("#msp-open-modal")?.addEventListener("click",()=>this._openModal())}_renderAll(){this._renderSubtitle(),this._renderStats(),this._renderList(),this._prices&&this._renderReplenishment()}_renderSubtitle(){const e=p.stack??[],t=w(e),s=this.container.querySelector("#msp-subtitle");s&&(s.textContent=`${e.length} suplemento${e.length!==1?"s":""} ativo${e.length!==1?"s":""} · ${y(t)}/mês estimado`)}_renderStats(){const e=this.container.querySelector("#msp-stats");if(!e)return;const t=p.stack??[],s=w(t),o=I(t);e.innerHTML=`
    ...
    `}_renderList(){const e=this.container.querySelector("#msp-list");if(!e)return;const t=p.stack??[];if(!t.length){e.innerHTML=`
        <div class="msp-empty">
      ...
      `,this.container.querySelector("#msp-empty-cta")?.addEventListener("click",()=>this._openModal());return}e.innerHTML="",t.forEach(s=>{const o=u(s),r=L(s),n=z(s),l=$(C(s)),d=document.createElement("div");d.className="msp-item",d.dataset.itemId=o;const a=x.find(_=>_.id===o),c=a?.category??"",v=a?.benefits?.[0]??"",h=k.getLinks(s.name);d.innerHTML=`
        <div class="msp-item-top">
          ...
          <div class="msp-item-info">
            ${c?`<p class="msp-item-cat">${c}</p>`:""}
            <p class="msp-item-name">${f(s.name)}</p>
          ...
          <div class="msp-item-right">
            ...
            <div class="msp-item-actions">
              <button class="msp-btn-icon" data-action="edit" data-id="${o}" aria-label="Editar ${s.name}" title="Editar">✏️</button>
              <button class="msp-btn-icon del" data-action="remove" data-id="${o}" aria-label="Remover ${s.name}" title="Remover">🗑️</button>
              <a class="msp-btn-reorder"
                 ...
                 aria-label="Recomprar ${f(s.name)} na Amazon">🛒 Recomprar</a>
            </div>
          </div>
        </div>
      ...
      `;const m=document.createElement("div");m.id=`msp-edit-${o}`,m.style.display="none",e.appendChild(d),e.appendChild(m)})}_renderReplenishment(){const e=this.container.querySelector("#msp-replenishment");if(!e)return;const t=p.stack??[],s=this._prices??{},o=t.filter(r=>{const n=u(r),l=s[n];return l&&Object.keys(l).length>0});if(!o.length){e.innerHTML='<p class="msp-replen-empty">Nenhum preço disponível para os itens do seu stack.</p>';return}e.innerHTML=o.map((r,n)=>{const l=u(r),d=s[l]??{},a=Object.values(d),c=a.reduce((h,m)=>h.price<m.price?h:m,a[0]),v=n<o.length-1?'<hr class="msp-replen-divider">':"";return`
        <div class="msp-replen-item">
          <span class="msp-replen-name">${f(r.name)}</span>
          <span class="msp-replen-price">Melhor: ${y(c.price)}</span>
          <span class="msp-replen-market">${c.label}</span>
        </div>
        ${v}
      `}).join("")}_attachDelegatedListeners(){this.container.querySelector("#msp-list")?.addEventListener("click",e=>{const t=e.target.closest("[data-aff-mp]");t&&k.trackClick(t.dataset.affId,t.dataset.affMp);const s=e.target.closest("[data-action]");if(!s)return;const o=s.dataset.id;if(s.dataset.action==="edit"&&this._toggleInlineEdit(o),s.dataset.action==="remove"){const r=(p.stack??[]).find(n=>(n.supplementId??n.id)===o);if(!r||!confirm(`Remover "${r.name}" do stack?`))return;p.dispatch(b.REMOVE_FROM_STACK,{supplementId:o})}s.dataset.action==="save-edit"&&this._saveInlineEdit(o),s.dataset.action==="cancel-edit"&&this._closeInlineEdit(o)})}_toggleInlineEdit(e){const t=this.container.querySelector(`#msp-edit-${e}`);if(!t)return;if(t.style.display!=="none"){this._closeInlineEdit(e);return}this.container.querySelectorAll('[id^="msp-edit-"]').forEach(o=>{o.id!==`msp-edit-${e}`&&(o.style.display="none")});const s=(p.stack??[]).find(o=>(o.supplementId??o.id)===e);s&&(t.style.display="block",t.innerHTML=`
      <div class="msp-inline-edit">
        <p class="msp-inline-edit-title">Editar — ${f(s.name)}</p>
