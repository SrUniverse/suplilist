import{s as l,A as g,o as k}from"./main-Q5JJQYYZ.js";import{S as u}from"./stack-recommender-b07295YU.js";import{r as _}from"./evidence-D5RtUc7g.js";function c(i){return i==null?null:i.supplementId??i.id??null}let m=null;async function E(){if(m)return m;try{m=await(await fetch("/data/prices.json")).json()}catch{m={}}return m}function f(i){return"R$ "+i.toFixed(2).replace(".",",")}function b(i){return i.reduce((e,t)=>{const a=u.find(n=>n.id===c(t))?.pricePerGram??0,r=parseFloat(t.dosage)||0;return e+r*a*30},0)}function w(i){if(!i.length||!(l.calculateStreak?.()??0))return"0%";const t=[];for(let n=0;n<7;n++)t.push(k(n));const s=l.checkins??[],a=new Set(s.map(n=>n.date)),r=t.filter(n=>a.has(n)).length;return Math.round(r/7*100)+"%"}function S(i){const e=u.find(s=>s.id===c(i));return e?.image?e.image:`/assets/${(i.name??"").toLowerCase().replace(/\s+/g,"_").replace(/[^a-z0-9_]/g,"")}.png`}function $(i){return u.find(t=>t.id===c(i))?.evidenceLevel??"C"}function I(i){const e=parseFloat(i.quantity),t=parseFloat(i.dosage);return!e||!t||t<=0?null:Math.max(0,Math.floor(e/t))}const C=`
  /* Layout */
  .msp-wrap {
    display: flex;
