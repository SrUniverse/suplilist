import{s as n,A as i}from"./main-Cc1NkQ61.js";import{r}from"./stack-recommender-DF0wTc9D.js";import{e as o}from"./escape-Br5wU8qn.js";const l=[{key:"bulk",emoji:"💪",label:"Hipertrofia"},{key:"strength",emoji:"⚡",label:"Força"},{key:"cut",emoji:"🔥",label:"Emagrecimento"},{key:"endurance",emoji:"🏃",label:"Resistência"},{key:"general",emoji:"🛡️",label:"Saúde Geral"}];class u{constructor(t){this.container=t,this.step=1,this.data={name:"",goal:null,selectedIds:new Set},this._suggestions=[],this._clickHandler=null,this._popstateHandler=null}mount(){this._popstateHandler=()=>{window.history.replaceState(null,null,"/onboarding")},window.addEventListener("popstate",this._popstateHandler),this._render()}unmount(){this._popstateHandler&&(window.removeEventListener("popstate",this._popstateHandler),this._popstateHandler=null),this._clickHandler&&(this.container.removeEventListener("click",this._clickHandler),this._clickHandler=null),this.container.innerHTML=""}_render(){switch(this._clickHandler&&(this.container.removeEventListener("click",this._clickHandler),this._clickHandler=null),this.step){case 1:this._renderStep1();break;case 2:this._renderStep2();break;case 3:this._renderStep3();break}this._clickHandler=t=>this._handleClick(t),this.container.addEventListener("click",this._clickHandler)}_renderStep1(){this.container.innerHTML=`
      ...
      </div>`;const t=this.container.querySelector(".onboarding-input"),a=this.container.querySelector(".onboarding-btn-next");t.addEventListener("input",()=>{this.data.name=t.value,a.disabled=!t.value.trim()}),setTimeout(()=>t.focus(),50)}_renderStep2(){const t=l.map(a=>`
      <button
      ...
      </div>`}_renderStep3(){this._suggestions.length||(this._suggestions=r.recommend({objective:this.data.goal}),this.data.selectedIds=new Set(this._suggestions.map(a=>a.id)));const t=this._suggestions.length?this._suggestions.map(a=>{const e=this.data.selectedIds.has(a.id);return`
            <div class="onboarding-supp-card${e?" selected":""}" data-supp="${a.id}">
              <div class="onboarding-supp-card__check">${e?"✓":""}</div>
              <div class="onboarding-supp-card__info">
                <div class="onboarding-supp-card__name">${o(a.name)}</div>
                <div class="onboarding-supp-card__meta">${o(a.category)} · ${a.dosage.daily}${a.dosage.unit}/dia · Evidência ${a.evidenceLevel??a.priority}</div>
              </div>
      ...
      </div>`}_handleClick(t){if(t.target.closest(".onboarding-btn-back")){this.step=Math.max(1,this.step-1),this._render();return}const a=t.target.closest("[data-goal]");if(a){if(a.dataset.goal===this.data.goal)return;this.data.goal=a.dataset.goal,this._suggestions=[],this.data.selectedIds=new Set,this.container.querySelectorAll(".onboarding-goal-card").forEach(d=>{d.classList.toggle("selected",d.dataset.goal===this.data.goal)});const s=this.container.querySelector(".onboarding-btn-next");s&&(s.disabled=!1);return}const e=t.target.closest("[data-supp]");if(e){const s=e.dataset.supp;this.data.selectedIds.has(s)?(this.data.selectedIds.delete(s),e.classList.remove("selected"),e.querySelector(".onboarding-supp-card__check").textContent=""):(this.data.selectedIds.add(s),e.classList.add("selected"),e.querySelector(".onboarding-supp-card__check").textContent="✓");return}t.target.closest(".onboarding-btn-next")&&(this.step<3?(this.step++,this._render()):this._submit())}_submit(){n.dispatch(i.SET_USER_PROFILE,{name:this.data.name.trim(),objective:this.data.goal}),this._suggestions.filter(t=>this.data.selectedIds.has(t.id)).forEach(t=>{n.dispatch(i.ADD_TO_STACK,{supplementId:t.id,name:t.name,dosage:t.dosage.daily,unit:t.dosage.unit})}),n.dispatch(i.COMPLETE_ONBOARDING),window.__router?.navigate("/my-stack")}}export{u as default};
