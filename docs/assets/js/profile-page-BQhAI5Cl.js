import{s as l,A as E,e as d}from"./main-Q5JJQYYZ.js";const $=[{value:"bulk",label:"Bulk",desc:"Ganho de Massa"},{value:"cut",label:"Cut",desc:"Definição Muscular"},{value:"strength",label:"Força",desc:"Força Máxima"},{value:"endurance",label:"Resistência",desc:"Cardio & Endurance"},{value:"general",label:"Saúde",desc:"Bem-estar Geral"}],f=["width:100%","padding:11px 14px","background:var(--color-bg-secondary)","border:1px solid var(--color-border)","color:var(--color-text-primary)","border-radius:10px","font-size:14px","font-family:inherit","box-sizing:border-box","outline:none","transition:border-color 0.15s"].join(";"),C=f+";appearance:none;cursor:pointer;";function m(h){return`<label style="display:block;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:var(--color-text-secondary);margin-bottom:6px;">${h}</label>`}function y(h,e,r=""){return`
  ...
  `}class _{constructor(e){this.container=e,this._form={},this._editingName=!1}mount(){const e=l.user||{};this._form={name:e.name||"Usuário",objective:e.objective||"general",weight:e.weight||"",height:e.height||"",age:e.age||""},this._render(),this._attachListeners()}unmount(){}_getObjectiveLabel(){const e=$.find(r=>r.value===this._form.objective);return e?e.label:"Saúde"}_getInitial(){return(this._form.name||"U").trim()[0].toUpperCase()}_render(){const e=this._form,r=this._getInitial(),p=this._getObjectiveLabel(),a=document.documentElement.getAttribute("data-theme")!=="light";this.container.innerHTML=`
        ...
        ${y("Dados Biométricos",`
          ...
          <div>
            ...
            <div style="position:relative;">
              <select id="field-objective" style="${C}">
                ${$.map(i=>`<option value="${i.value}" ${e.objective===i.value?"selected":""}>${i.label} — ${i.desc}</option>`).join("")}
              </select>
