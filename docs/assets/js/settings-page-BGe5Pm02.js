import{N as k,s as d,S as r,a as x,A as m}from"./main-BRQQPoec.js";import{C as y}from"./checkout-modal-Djl4Fzqa.js";class A{constructor(e,o){this.container=e,this.params=o,this.notifService=new k}mount(){this._injectStyles();const e=d.state;this._lastTier=e.user?.tier??"free",this.container.innerHTML=this._render(),this._bindEvents(),this._unsubscribe=d.subscribe(()=>{const s=d.state.user?.tier??"free";if(s===this._lastTier)return;this._lastTier=s;const n=this.container.querySelector(".sp-subscription-section");n&&(n.outerHTML=`<div class="sp-subscription-section">${this._renderSubscriptionSection(s)}</div>`,this._bindSubscriptionEvents())})}unmount(){this._unsubscribe?.(),this.container.innerHTML=""}_injectStyles(){if(document.getElementById("settings-page-styles"))return;const e=document.createElement("style");e.id="settings-page-styles",e.textContent=`
      .sp-page {
        padding: 24px;
        max-width: 700px;
        display: flex;
