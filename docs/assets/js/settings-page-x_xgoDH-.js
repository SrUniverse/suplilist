import{N as v,S as s,a as m,s as x,A as k}from"./main-CJjCf5yQ.js";class E{constructor(t,o){this.container=t,this.params=o,this.notifService=new v}mount(){this._injectStyles(),this.container.innerHTML=this._render(),this._bindEvents()}unmount(){this.container.innerHTML=""}_injectStyles(){if(document.getElementById("settings-page-styles"))return;const t=document.createElement("style");t.id="settings-page-styles",t.textContent=`
      .sp-page {
        padding: 24px;
        max-width: 700px;
        display: flex;
    ...
    `,document.head.appendChild(t)}_getThemeState(){const t=s.getItem(m.THEME);return t?t==="dark":document.documentElement.getAttribute("data-theme")==="dark"}_getBoolPref(t){return s.getItem(t)==="true"}_switchHTML(t,o,r,p){return`
    ...
    `}_bindEvents(){const t=this.container.querySelector("#sp-theme-toggle");t&&t.addEventListener("change",()=>{const e=t.checked,c=e?"dark":"light";document.documentElement.setAttribute("data-theme",c),s.setItem(m.THEME,c);const d=this.container.querySelector("#sp-theme-toggle-icon");d&&(d.textContent=e?"🌙":"☀️");const a=e?'<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>':'<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>',g=document.querySelector("#btn-theme .sb-item__icon");g&&(g.innerHTML=a);const f=document.getElementById("btn-theme-mobile");f&&(f.innerHTML=a)});const o=this.container.querySelector("#sp-notif-checkin");o&&o.addEventListener("change",async()=>{if(o.checked){if(!await this.notifService.requestPermission()){alert("Permissão de notificações negada. Por favor, ative as notificações nas configurações do seu navegador para receber lembretes."),o.checked=!1;return}this.notifService.sendLocalNotification("Lembretes Ativados! 💊",{body:"Agora você receberá lembretes diários para não esquecer seus suplementos.",data:{url:"/settings"}})}s.setItem("suplilist:notif-checkin",o.checked?"true":"false")});const r=this.container.querySelector("#sp-notif-restock");r&&r.addEventListener("change",async()=>{if(r.checked){if(!await this.notifService.requestPermission()){alert("Permissão de notificações negada. Por favor, ative as notificações nas configurações do seu navegador para receber alertas de estoque."),r.checked=!1;return}this.notifService.sendLocalNotification("Alertas de Estoque Ativados! 📦",{body:"Você receberá avisos quando seus suplementos estiverem acabando.",data:{url:"/settings"}})}s.setItem("suplilist:notif-restock",r.checked?"true":"false")});const p=this.container.querySelector("#sp-export-btn");p&&p.addEventListener("click",()=>{const e={},c=s.getAllKeys();for(const a of c)if(a&&a.startsWith("suplilist"))try{e[a]=JSON.parse(s.getItem(a))}catch{e[a]=s.getItem(a)}const d=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),u=URL.createObjectURL(d),l=document.createElement("a");l.href=u,l.download=`suplilist-export-${new Date().toISOString().slice(0,10)}.json`,document.body.appendChild(l),l.click(),document.body.removeChild(l),setTimeout(()=>URL.revokeObjectURL(u),1e3)});const n=this.container.querySelector("#sp-export-file-btn");n&&s.isFileSystemAPIAvailable()&&(this.container.querySelector("#sp-export-file-row").style.display="flex",n.addEventListener("click",async()=>{n.disabled=!0,n.textContent="Salvando...";try{const e=await s.exportToFile();alert(e.message),e.success&&(n.textContent="✓ Salvo",setTimeout(()=>{n.textContent="Salvar"},2e3))}finally{n.disabled=!1}}));const i=this.container.querySelector("#sp-import-file-btn");i&&s.isFileSystemAPIAvailable()&&(this.container.querySelector("#sp-import-file-row").style.display="flex",i.addEventListener("click",async()=>{i.disabled=!0,i.textContent="Restaurando...";try{const e=await s.importFromFile();alert(e.message),e.success&&(i.textContent="✓ Restaurado",setTimeout(()=>{i.textContent="Restaurar",location.reload()},2e3))}finally{i.disabled=!1}}));const b=this.container.querySelector("#sp-clear-checkins-btn");b&&b.addEventListener("click",()=>{confirm("Deseja limpar todo o histórico de check-ins? Esta ação não pode ser desfeita.")&&x.dispatch(k.CLEAR_CHECKINS)});const h=this.container.querySelector("#sp-reset-btn");h&&h.addEventListener("click",()=>{confirm("⚠️ ATENÇÃO: Isso vai apagar TODOS os seus dados (stack, check-ins, perfil). Não há como desfazer.")&&(Object.keys(localStorage).filter(e=>e.startsWith("suplilist")).forEach(e=>s.removeItem(e)),location.reload())}),this.container.querySelectorAll(".sp-link-btn[data-path]").forEach(e=>{e.addEventListener("click",()=>{const c=e.getAttribute("data-path");window.history.pushState(null,null,c),window.dispatchEvent(new PopStateEvent("popstate"))})})}}export{E as default};
