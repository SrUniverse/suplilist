# Supplements DB Complete — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Substituir 90 placeholders no SUPPLEMENTS_DB por 45 entradas reais, renomear `cafeina` → `cafeina-teanina`, corrigir `list-page.js` para usar `item.image`, e sincronizar o JSON legado.

**Architecture:** O source of truth é `src/ai/stack-recommender.js` (exporta `SUPPLEMENTS_DB`). Todas as páginas importam daí. `public/data/supplements-db.json` é orphaned (não importado por nenhum código) — será sincronizado por último. A lista final tem 55 entradas com campo `image` explícito em cada uma.

**Tech Stack:** Vanilla JS (ES modules), Vitest para testes, JSON estático.
...
| Arquivo | Mudança |
|---|---|
| `src/ai/stack-recommender.js` | Renomear `cafeina` → `cafeina-teanina` + adicionar 45 entradas novas |
| `src/pages/list-page.js` | 2 linhas: usar `item.image` em vez de derivar slug |
| `src/ai/stack-recommender.test.js` | Atualizar teste de IDs obrigatórios + adicionar teste de campo `image` |
| `public/data/supplements-db.json` | Sincronizar com o JS (55 entradas reais) |

---

## Task 1: Corrigir list-page.js

**Files:**
- Modify: `src/pages/list-page.js:731` e `:839`

- [ ] **Step 1: Verificar as linhas atuais**

```
# Linha 731 (card render):
const slug = item.id.replace(/-/g, '_');
# Linha 839 (modal render):
const slug = item.id.replace(/-/g, '_');
```

- [ ] **Step 2: Corrigir linha 731**

Substituir em `src/pages/list-page.js` linha 731:
```js
// ANTES:
const slug = item.id.replace(/-/g, '_');
// DEPOIS:
...
E na linha 756 mudar `src="/assets/${slug}.png"` para `src="${img}"`.

- [ ] **Step 3: Corrigir linha 839 (modal)**

Substituir em `src/pages/list-page.js` linha 839:
```js
// ANTES:
const slug = item.id.replace(/-/g, '_');
// DEPOIS:
...
E na linha 890 mudar `src="/assets/${slug}.png"` para `src="${img}"`.

- [ ] **Step 4: Commit**

```bash
git add src/pages/list-page.js
git commit -m "fix(list): use item.image field instead of deriving path from slug"
```
...
- [ ] **Step 1: Substituir o teste de IDs obrigatórios**

Substituir o bloco do teste `'Validation Checklist: All 10 mandatory supplement IDs present in SUPPLEMENTS_DB'` pelo seguinte:
...
- [ ] **Step 2: Adicionar teste de campo image**

Adicionar após o teste acima:
...
it('Validation: All supplements have image field pointing to /assets/', () => {
  SUPPLEMENTS_DB.forEach(s => {
    ...
    expect(s.image, `${s.id} image must start with /assets/`).toMatch(/^\/assets\/.+\.png$/);
  });
});
```

- [ ] **Step 3: Rodar testes (devem falhar — ainda não adicionamos os dados)**

```bash
npx vitest run src/ai/stack-recommender.test.js
```
Expected: FAIL — IDs não encontrados.

- [ ] **Step 4: Commit (testes falhando — intencionalmente)**

```bash
git add src/ai/stack-recommender.test.js
git commit -m "test(supplements): update mandatory IDs to 55 + add image field validation"
```

---

## Task 3: Renomear cafeina → cafeina-teanina no stack-recommender.js

**Files:**
- Modify: `src/ai/stack-recommender.js`

- [ ] **Step 1: Alterar entrada `cafeina` no SUPPLEMENTS_DB**

Em `src/ai/stack-recommender.js`, localizar o objeto com `id: 'cafeina'` e fazer as seguintes trocas:
...
Também atualizar os campos de conteúdo para refletir a combinação:
```js
benefits: [
  'Foco e alerta mental sem a ansiedade da cafeína isolada',
  'Sinergia comprovada: teanina suaviza os efeitos estimulantes da cafeína',
  'Melhora de performance cognitiva e física pré-treino'
],
warnings: [
  'Evite consumir após as 18h para não prejudicar o sono',
  'Não exceda a dose máxima de cafeína (400mg/dia)'
],
```

- [ ] **Step 2: Verificar se `cafeina` é referenciado em outros arquivos**

```bash
grep -r "cafeina" src/ --include="*.js" -l
```

Se encontrar referências ao ID `'cafeina'` (não ao nome), atualizar para `'cafeina-teanina'`.
...
npx vitest run src/ai/stack-recommender.test.js
```
Expected: Still FAIL (demais IDs ausentes), mas o teste de `cafeina` do `cut profile` deve agora referenciar `cafeina-teanina`.

- [ ] **Step 4: Atualizar teste que referencia 'cafeina'**

Em `stack-recommender.test.js`, teste `"2. Cut 65kg → results include 'l-carnitina' or 'cafeina'"`:
```js
// ANTES:
...
```bash
git add src/ai/stack-recommender.js src/ai/stack-recommender.test.js
git commit -m "feat(supplements): rename cafeina → cafeina-teanina, update to Cafeína + L-Teanina"
```

---

## Task 4: Adicionar 45 novos suplementos ao SUPPLEMENTS_DB

**Files:**
- Modify: `src/ai/stack-recommender.js` — append após o bloco `ashwagandha`

- [ ] **Step 1: Adicionar os 45 novos suplementos**

Inserir as seguintes entradas no array `SUPPLEMENTS_DB` após o objeto `ashwagandha` (antes do fechamento `];`):
...
```js
  ...
  },
  {
    ...
    warnings: ['Fraccione em doses ≤500mg — absorção intestinal limitada por dose', 'Excesso crônico aumenta risco de cálculos renais'],
  ...
  },
  {
    ...
    pricePerGram: 0.3,
    safetyScore: 92,
    benefits: ['Potente anti-inflamatório sistêmico (inibidor de NF-κB)', 'Aceleração da recuperação muscular pós-treino', 'Proteção articular e neuroproteção'],
  ...
  },
  {
    ...
    pricePerGram: 0.3,
    safetyScore: 92,
    benefits: ['Estimulação da produção de NGF (fator de crescimento nervoso)', 'Melhora de memória e cognição em estudos clínicos', 'Neuroproteção e potencial em prevenção neurodegenerativa'],
  ...
  },
  {
    ...
    pricePerGram: 0.06,
    safetyScore: 96,
    benefits: ['Aumento de óxido nítrico e vasodilatação (pump muscular)', 'Redução de fadiga e aumento de repetições máximas', 'Superior à arginina em biodisponibilidade oral'],
  ...
  },
  {
    ...
    pricePerGram: 0.05,
    safetyScore: 84,
    benefits: ['Tônico sexual e afrodisíaco da medicina tradicional amazônica', 'Efeito adaptogênico para fadiga física e mental', 'Potencial neuroprotetor em estudos preliminares'],
  ...
  },
  {
    ...
    warnings: ['Menos é mais — 0.5-1mg são frequentemente mais eficazes que 5-10mg', 'Não use cronicamente sem necessidade — pode reduzir produção endógena'],
  ...
  },
  {
    ...
    warnings: ['Prefira produtos com múltiplas cepas e contagem ≥10 bilhões UFC', 'Armazene conforme instrução — muitos requerem refrigeração'],
  ...
  },
  {
    ...
    restrictions: [],
    dosage: { isFixed: true, maintenance: 5000, upperLimit: 15000, unit: 'mg', timing: '30 min antes das refeições principais com bastante água (≥300ml)' },
    pricePerGram: 0.01,
    safetyScore: 97,
  ...
  },
  {
    ...
    pricePerGram: 0.1,
    safetyScore: 88,
    benefits: ['Tônico reprodutivo feminino da medicina Ayurvédica', 'Modulação do sistema hormonal feminino (fase lútea)', 'Efeito adaptogênico geral e suporte imunológico'],
  ...
  },
  {
    ...
    warnings: ['Contaminação por microcistinas em produtos de baixa qualidade — compre de fontes certificadas', 'Não é substituta de B12 para veganos — biodisponibilidade controversa'],
  ...
  },
  {
    ...
    pricePerGram: 0.02,
    safetyScore: 97,
    benefits: ['Melhora da resistência muscular e redução de câibras', 'Suporte à saúde cardiovascular e regulação da pressão arterial', 'Proteção osmótica celular em esforços intensos'],
  ...
  },
  {
    ...
    warnings: ['Use extrato padronizado 200:1 (Eurycomanone ≥1%)', 'Cicle 5 dias on / 2 dias off para preservar eficácia'],
...
- [ ] **Step 2: Verificar que o array fecha corretamente**

Confirmar que após o último objeto (`zinco-bisglicinato`) há `];` encerrando o array.
...
npx vitest run src/ai/stack-recommender.test.js
```
Expected: PASS em todos os testes (incluindo os 2 novos de IDs e image).

- [ ] **Step 4: Verificar contagem**

```bash
node -e "import('./src/ai/stack-recommender.js').then(m => console.log('Total:', m.SUPPLEMENTS_DB.length))"
```
Expected: `Total: 55`
...
```bash
git add src/ai/stack-recommender.js
git commit -m "feat(supplements): add 45 real supplement entries — DB complete with 55 supplements"
```

---

## Task 5: Sincronizar public/data/supplements-db.json

**Files:**
- Modify: `public/data/supplements-db.json`

O JSON não é importado por nenhum código JS (verificado via grep). Serve como referência estática / futura integração. Substituir todo o conteúdo pelo array gerado a partir do SUPPLEMENTS_DB.

- [ ] **Step 1: Gerar o JSON sincronizado**

```bash
node -e "
import('./src/ai/stack-recommender.js').then(m => {
  const fs = await import('fs');
  fs.writeFileSync('public/data/supplements-db.json', JSON.stringify(m.SUPPLEMENTS_DB, null, 2));
  console.log('Synced', m.SUPPLEMENTS_DB.length, 'supplements');
});
"
```

Ou alternativamente, copiar manualmente o conteúdo do array para o JSON (remover sintaxe JS, garantir aspas duplas em todas as keys).

- [ ] **Step 2: Verificar o JSON**

```bash
node -e "const d = require('./public/data/supplements-db.json'); console.log(d.length, 'supplements, first:', d[0].id)"
```
Expected: `55 supplements, first: creatina-monohidratada`

- [ ] **Step 3: Commit**

```bash
git add public/data/supplements-db.json
git commit -m "chore(data): sync supplements-db.json with JS source of truth (55 entries)"
```
...
**Spec coverage:**
- ✅ 45 novos suplementos adicionados ao SUPPLEMENTS_DB
- ✅ `cafeina` → `cafeina-teanina` renomeado (ID, nome, conteúdo)
- ✅ `list-page.js` corrigido para usar `item.image`
- ✅ Testes atualizados (55 IDs + validação de campo image)
- ✅ JSON sincronizado

**Placeholders:** Nenhum TBD ou TODO no plano.

**Type consistency:** Todos os objetos seguem o mesmo schema que os 10 existentes no SUPPLEMENTS_DB.

**Edge cases:**
- `glucosamina-condroitina` tem `restrictions: ['shellfish']` — válido segundo o schema
- `melatonina` tem `maintenance: 0.5` (menos de 1mg) — válido, o campo aceita float
- `probiotico` usa `unit: 'bi UFC'` — string customizada, funciona no display
