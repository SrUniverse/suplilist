...
const DOCS_DIR = path.join(__dirname, '../dist');
...
console.log('Carregando banco de dados de suplementos...');
const supplementsData = fs.readFileSync(path.join(__dirname, '../public/data/supplements-db.json'), 'utf-8');
...
console.log('Iniciando a Pré-renderização Estática...');
  ...
  <main id="router-outlet" role="main" aria-live="polite" aria-label="Conteúdo principal">
    <article style="max-width: 800px; margin: 40px auto; padding: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #F2F2F2; background: #0F0F0F; border: 1px solid rgba(255,255,255,0.07); border-radius: 16px;">
      ...
      <div style="margin-top: 40px; border-top: 1px solid rgba(255,255,255,0.07); padding-top: 24px; text-align: center;">
        ...
        <a href="https://suplilist.com/list?id=${supp.id}" style="display: inline-block; background: #7C3AED; color: #FFFFFF; text-decoration: none; padding: 12px 24px; border-radius: 8px; font-weight: 600;">
...
console.log('Regerando o sitemap.xml...');
...
console.log(`✓ sitemap.xml atualizado com ${generatedUrls.length} URLs!`);
