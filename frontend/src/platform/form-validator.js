...
class FormValidator {
   ...
   * @returns {string | null}
   */
  validateField(fieldName, value, ruleString) {
    const rules = ruleString.split('|');

    for (const rule of rules) {
      const [ruleType, ...ruleParams] = rule.split(':');
      const error = this.validateRule(fieldName, value, ruleType, ruleParams);
   ...
   * @returns {string | null}
   */
  validateRule(fieldName, value, ruleType, params) {
    const fieldLabel = this.humanizeFieldName(fieldName);

    switch (ruleType) {
      case 'required':
        if (!value || (typeof value === 'string' && value.trim() === '')) {
          return this.messages.get('required')?.replace('{field}', fieldLabel) || 'Campo obrigatório';
        }
        break;
      ...
      case 'max':
        if (value && value.length > parseInt(params[0], 10)) {
          return this.messages
            ...
            ?.replace('{field}', fieldLabel)
            .replace('{param}', params[0])
            || `Máximo ${params[0]} caracteres`;
        }
        break;

      case 'match': {
        // match validation requires the full data object — use validateWithData() instead
        void params;
        break;
      }
      ...
      case 'phone':
        if (value && !this.isValidPhone(value)) {
          return this.messages.get('phone') || 'Telefone inválido';
        }
        break;
