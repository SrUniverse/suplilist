...
// ══════════════════════════════════════════════════════════════
export function getProductsWithLinks(products) {
  if (!products || !Array.isArray(products)) {
    return [];
  }
  let processed = 0, skipped = 0;
  const updatedProducts = products.map(function(item) {
    const newItem = { ...item };
    ...
    const lk = PRODUCT_LINKS[newItem.id];

    // [SL-06] Se não há entrada no dicionário, aplica fallback de busca e preserva dados originais
    if (!lk || typeof lk !== 'object') {
      newItem.linkAmazon = newItem.linkAmazon || searchUrl(newItem.name, 'amazon');
      newItem.linkML     = newItem.linkML     || searchUrl(newItem.name, 'ml');
      newItem.linkShopee = newItem.linkShopee || newItem.shopee || searchUrl(newItem.name, 'shopee');
      newItem.isAmazonDirect = false;
      newItem.isMLDirect     = false;
      newItem.isShopeeDirect = !!newItem.shopee;
      skipped++;
      return newItem;
    }

    // Garantia de propriedades para evitar undefined em cascata
    lk.amazon = lk.amazon || "";
    lk.ml     = lk.ml || "";
    lk.shopee = lk.shopee || "";

    // Preço: sobrescreve apenas se o valor em links.js for um número válido
    if (lk.pm && typeof lk.pm === 'number' && lk.pm > 0) newItem.pm = lk.pm;

    // Amazon — aplica tag de afiliado; fallback para busca
    newItem.linkAmazon = lk.amazon ? amazonAff(lk.amazon) : searchUrl(newItem.name, 'amazon');
    newItem.isAmazonDirect = !!lk.amazon;

    // Mercado Livre — aplica UTM de afiliado; fallback para busca
    newItem.linkML = lk.ml ? mlAff(lk.ml, newItem.id) : searchUrl(newItem.name, 'ml');
    newItem.isMLDirect = !!lk.ml;

    // Shopee — aplica UTM de afiliado; fallback para link direto ou busca
    newItem.linkShopee = lk.shopee ? shopeeAff(lk.shopee, newItem.id) : (newItem.shopee || searchUrl(newItem.name, 'shopee'));
    newItem.isShopeeDirect = !!lk.shopee;
