// ============================================================
// ProfilePage v4.0 — SupliList
// Formulário real que lê e salva no stateManager.
// ============================================================
...
export default class ProfilePage {
  constructor(container) {
    ...
    this._listeners = [];
    // Working copy of the form — only saved on submit
    this._form = {};
  }
  ...
  _inputStyle() {
    return 'width:100%;padding:11px 12px;background:var(--color-bg-secondary);border:1px solid var(--color-border);color:var(--color-text-primary);border-radius:10px;font-size:14px;font-family:inherit;';
  }
      ...
      <div id="profile-root" style="padding:20px 16px;display:flex;flex-direction:column;gap:20px;padding-bottom:40px;">
        ...
        <form id="profile-form" style="display:flex;flex-direction:column;gap:20px;">
          ...
          <div style="background:var(--color-surface-primary);border:1px solid var(--color-border);border-radius:16px;padding:18px;display:flex;flex-direction:column;gap:14px;">
            ...
            <div>
              ${this._label('Nome')}
              <input id="field-name" type="text" value="${f.name}" placeholder="Seu nome" style="${this._inputStyle()}" />
            </div>
          </div>
          ...
          <div style="background:var(--color-surface-primary);border:1px solid var(--color-border);border-radius:16px;padding:18px;display:flex;flex-direction:column;gap:14px;">
            ...
            <div style="display:flex;flex-direction:column;gap:8px;">
              ${OBJECTIVES.map(obj => `
                <label style="display:flex;align-items:center;gap:12px;cursor:pointer;padding:12px;background:${f.objective === obj.value ? 'var(--color-brand-muted)' : 'var(--color-bg-secondary)'};border:1px solid ${f.objective === obj.value ? 'var(--color-brand)' : 'transparent'};border-radius:10px;transition:all 0.15s;">
                  <input type="radio" name="objective" value="${obj.value}" ${f.objective === obj.value ? 'checked' : ''} style="accent-color:var(--color-brand);width:16px;height:16px;" />
                  <span style="font-size:14px;font-weight:${f.objective === obj.value ? '700' : '500'};color:${f.objective === obj.value ? 'var(--color-brand)' : 'inherit'};">${obj.label}</span>
                </label>
              `).join('')}
            </div>
          </div>
          ...
          <div style="background:var(--color-surface-primary);border:1px solid var(--color-border);border-radius:16px;padding:18px;display:flex;flex-direction:column;gap:14px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
              <div>
                ${this._label('Peso (kg)')}
                <input id="field-weight" type="number" min="30" max="250" value="${f.weight}" style="${this._inputStyle()}" />
              </div>
              <div>
                ${this._label('Treinos/semana')}
                <input id="field-freq" type="number" min="1" max="7" value="${f.trainingFrequency}" style="${this._inputStyle()}" />
              </div>
            </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
              <div>
                ...
                <input id="field-training-age" type="number" min="0" max="50" value="${f.trainingAge}" style="${this._inputStyle()}" />
              </div>
              <div>
                ${this._label('Budget mensal (R$)')}
                <input id="field-budget" type="number" min="0" max="5000" value="${f.budget}" style="${this._inputStyle()}" />
              </div>
            </div>
          </div>
  ...
  _attachListeners() {
    // Text/number inputs — atualizam _form sem salvar
    const fields = [
    ...
    this.container.querySelectorAll('input[name="objective"]').forEach(radio => {
      this._on(radio, 'change', () => {
        this._form.objective = radio.value;
        // Atualizar visual sem re-render completo
        this.container.querySelectorAll('label:has(input[name="objective"])').forEach(label => {
          const val = label.querySelector('input').value;
          const active = val === this._form.objective;
          label.style.background = active ? 'var(--color-brand-muted)' : 'var(--color-bg-secondary)';
          label.style.borderColor = active ? 'var(--color-brand)' : 'transparent';
          label.querySelector('span').style.fontWeight = active ? '700' : '500';
          label.querySelector('span').style.color = active ? 'var(--color-brand)' : 'inherit';
        });
      });
    });
    ...
    this.container.querySelectorAll('.btn-restriction').forEach(btn => {
      this._on(btn, 'click', () => {
        ...
        if (idx === -1) {
          // #7 FIX: spread imutável — sem push() direto no array
          this._form.restrictions = [...this._form.restrictions, val];
        } else {
          // #7 FIX: filter imutável — sem splice() direto no array
          this._form.restrictions = this._form.restrictions.filter((_, i) => i !== idx);
        }
        // Re-render só a seção de restrições seria ideal, mas re-render completo é mais simples e seguro
        this.unmount();
        this._render();
        this._attachListeners();
      });
